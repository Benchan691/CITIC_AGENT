...
def register_tools(server, *, get_runtime, fresh_runtime, execute) -> None:
    ...
    async def zimbra_get_email_filter(ctx: Context, name: str, account_id: str = "") -> dict[str, Any]:
        ...
        return await execute(ctx, "zimbra", "get_email_filter", lambda: get_runtime(ctx).zimbra_filters.get_email_filter(name, account_id))

    @server.tool()
    async def zimbra_validate_email_filter(ctx: Context, rule: dict[str, Any], account_id: str = "") -> dict[str, Any]:
        """Validate a structured Zimbra filter without writing it."""
        return await execute(ctx, "zimbra", "validate_email_filter", lambda: get_runtime(ctx).zimbra_filters.validate_email_filter(rule, account_id))

    @server.tool()
    async def zimbra_preview_email_filter_update(ctx: Context, name: str, proposed_rule: dict[str, Any], account_id: str = "") -> dict[str, Any]:
        """Preview a structured Zimbra filter update without writing it."""
        return await execute(ctx, "zimbra", "preview_email_filter_update", lambda: get_runtime(ctx).zimbra_filters.preview_email_filter_update(name, proposed_rule, account_id))

    @server.tool()
    async def zimbra_create_email_filter(ctx: Context, rule: dict[str, Any], expected_fingerprint: str, account_id: str = "") -> dict[str, Any]:
        """Create a Zimbra filter only when write and dangerous-action gates permit it."""
        return await execute(ctx, "zimbra", "create_email_filter", lambda: get_runtime(ctx).zimbra_filters.create_email_filter(rule, expected_fingerprint, account_id))

    @server.tool()
    async def zimbra_update_email_filter(ctx: Context, name: str, rule: dict[str, Any], expected_fingerprint: str, account_id: str = "") -> dict[str, Any]:
    ...
    @server.tool()
    async def zimbra_reorder_email_filter(ctx: Context, name: str, order: int, expected_fingerprint: str, account_id: str = "") -> dict[str, Any]:
        """Move a Zimbra filter to a 1-based position using optimistic concurrency protection."""
        return await execute(ctx, "zimbra", "reorder_email_filter", lambda: get_runtime(ctx).zimbra_filters.reorder_email_filter(name, order, expected_fingerprint, account_id))
