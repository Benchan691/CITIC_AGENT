...
test('CITIC SOC defers generic Splunk background until a Splunk tool is visible', () => {
  ...
  assert.match(detection, /CITIC Alert Delivery/)
  assert.match(detection, /AID/)
  assert.doesNotMatch(detection, /action\.logevent=1/)
  assert.doesNotMatch(detection, /\.\.\.\s*\|\s*outputcsv/)
  assert.doesNotMatch(detection, /`(?:Event_GID|Event_Rulenum|GID)`\s*=/)
...
test('SOC skills are concise, scoped, and use bounded action parameters', () => {
  ...
  assert.match(splWriting, /CITIC Alert Delivery/)
  assert.match(splWriting, /EID/)
  assert.doesNotMatch(splWriting, /\.\.\.\s*\|\s*outputcsv/)
})
