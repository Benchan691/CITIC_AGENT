...
test('CITIC SOC loads generic Splunk background with the startup instructions', () => {
  ...
  const investigation = readFileSync(join(repoRoot, 'skills', 'splunk-investigation', 'SKILL.md'), 'utf8')
  ...
  assert.match(background, /RuleName_EN.*RuleNum_Name/is)
  assert.match(background, /GID.*Default/is)
  assert.doesNotMatch(background, /\[COMPANY_SHORT\]|\[Fubon\]/)
  assert.match(background, /Ruleset\.csv/)
  assert.match(background, /must\s+not be treated as a\s+customer abbreviation/i)
  assert.match(investigation, /Description_EN/)
  assert.match(investigation, /^---\nname: splunk-investigation\ndescription:.*read-only Splunk investigations/m)
  assert.match(investigation, /Remediation_EN/)
  assert.match(investigation, /GID.*Default/is)
  assert.match(investigation, /RuleNum.*0000.*9999/is)
  assert.match(investigation, /Severity.*Low.*Medium.*High.*Critical/is)
  assert.match(investigation, /read-only.*never.*reserved/is)
  assert.match(investigation, /customer-scoped/i)
  assert.doesNotMatch(investigation, /\[COMPANY_SHORT\]|\[Fubon\]/)
  assert.doesNotMatch(investigation, /catalog_(?:list|write|update|archive)/i)
