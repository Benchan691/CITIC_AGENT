...
- Inspect an existing rule before modifying, enabling, or disabling it.
- New and modified rules remain disabled until separately reviewed and approved.
- Validate before every backtest or write.
- Backtests are bounded samples, not proof of production quality.
...
- Test with `splunk_backtest_detection` using a bounded period and result count.
- Stage with `splunk_create_detection_draft` or `splunk_update_detection_draft`.
...
6. Backtest on a representative bounded period. Examine match count, samples, repeated entities, field consistency, noise, suppression need, and performance.
...
9. Create or update a disabled draft through approval.
10. Enable only through a second, explicit approval after review. Verify resulting state.
11. If behavior is unsafe or noisy, disable and document rollback evidence.
