#!/usr/bin/env python3
"""Comprehensive SSE transport tests for Splunk MCP server."""
...
async def test_sse_transport():
    ...
    print(f"{BOLD}{CYAN}Running SSE Transport Tests{RESET}")
    print("=" * 50)
    ...
    try:
        async with sse_client(server_url) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                try:
                    ...
                    expected_tools = ['search_oneshot', 'search_export', 'get_indexes', 
                                    'get_saved_searches', 'run_saved_search', 'get_config']
                ...
                # Test 4: Search with different output formats
                ...
                print(f"\n{BOLD}Testing get_saved_searches{RESET}")
                try:
                    result = await session.call_tool("get_saved_searches", arguments={})
                    saved_searches = json.loads(result.content[0].text)
                    if 'saved_searches' in saved_searches:
                        results.add_pass(f"get_saved_searches - Found {saved_searches['count']} searches")
                    else:
                        results.add_fail("get_saved_searches", "Invalid response format")
                except Exception as e:
                    results.add_fail("get_saved_searches", str(e))
                
                # Test 7: Resources
                print(f"\n{BOLD}Testing Resources{RESET}")
                resources = ["splunk://saved-searches", "splunk://indexes"]
        ...
        print(f"\n{RED}Fatal error connecting to server:{RESET} {str(e)}")
        print("Make sure the server is running on port 8050 with SSE transport")
        return False
    ...
    if success:
        print(f"\n{GREEN}{BOLD}All SSE transport tests passed!{RESET}")
    else:
...
if __name__ == "__main__":
    success = asyncio.run(test_sse_transport())
