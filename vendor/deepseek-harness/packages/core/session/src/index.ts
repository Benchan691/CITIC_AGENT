...
function validateSessionHeader(id: SessionId, input: unknown): SessionHeader {
  ...
  if (record.folderId !== undefined && typeof record.folderId !== 'string') {
    throw new Error('session header folderId must be a string')
  }
...
export class SessionStore extends Service {
  ...
  /**
   * Delete one live session from the store and emit its normal disposal edge.
   * Durable persistence is owned by the persistence service; callers that need
   * a complete deletion must remove the durable log through that service too.
   * Repeated deletion is an idempotent no-op.
