...
import { open, mkdir, readFile, readdir, realpath, link, rm, stat, truncate, rename, writeFile } from 'node:fs/promises'
...
import {
  ...
  type SessionFolderRecord,
  type StoredPrefix,
} from '@deepseek-ai/dsh-session-persistence'
import type { FolderId, SessionEvent, SessionId, SessionHeader, SessionPreparation } from '@deepseek-ai/dsh-session'
import {
...
interface JsonlFolderState {
  folders: SessionFolderRecord[]
  sessionFolders: Record<string, FolderId>
}
...
export class JsonlSessionPersistence extends SessionPersistence implements PersistenceBackend<JsonlTornMarker> {
  ...
  private folderState?: JsonlFolderState
  private folderTail: Promise<void> = Promise.resolve()
  ...
  async list(signal?: AbortSignal): Promise<SessionHeader[]> {
    const state = await this.readFolderState()
    return (await this.listArtifacts(signal)).map((artifact) => {
      const folderId = state.sessionFolders[String(artifact.header.id)] ?? artifact.header.folderId
      return folderId === undefined ? artifact.header : { ...artifact.header, folderId }
    })
  }

  override async listFolders(): Promise<SessionFolderRecord[]> {
    return (await this.readFolderState()).folders.map(folder => ({ ...folder }))
  }

  override async createFolder(folder: SessionFolderRecord): Promise<void> {
    await this.updateFolderState((state) => {
      if (state.folders.some(existing => existing.id === folder.id)) throw new Error(`folder '${folder.id}' already exists`)
      state.folders.push({ ...folder })
    })
  }

  override async renameFolder(id: FolderId, name: string, description?: string): Promise<void> {
    await this.updateFolderState((state) => {
      const folder = state.folders.find(existing => existing.id === id)
      if (folder === undefined) throw new Error(`folder '${id}' not found`)
      folder.name = name
      if (description === undefined) delete folder.description
      else folder.description = description
      folder.updatedAt = Date.now()
    })
  }

  override async deleteFolder(id: FolderId): Promise<boolean> {
    return await this.updateFolderState((state) => {
      const before = state.folders.length
      state.folders = state.folders.filter(folder => folder.id !== id)
      for (const [sessionId, folderId] of Object.entries(state.sessionFolders)) {
        if (folderId === id) delete state.sessionFolders[sessionId]
      }
      return state.folders.length !== before
    })
  }

  override async setSessionFolder(id: SessionId, folderId: FolderId): Promise<void> {
    await this.updateFolderState((state) => {
      if (!state.folders.some(folder => folder.id === folderId)) throw new Error(`folder '${folderId}' not found`)
      state.sessionFolders[String(id)] = folderId
    })
  }

  /** Delete one session log; the log contains both the header and all messages/events. */
  override async deleteSession(id: SessionId): Promise<boolean> {
    await this.ensureRootEncoding()
    const path = await this.findLog(id)
    if (path === undefined) {
      await this.updateFolderState((state) => { delete state.sessionFolders[String(id)] })
      return false
    }
    await rm(path)
    await this.updateFolderState((state) => { delete state.sessionFolders[String(id)] })
  ...
  private async readFolderState(): Promise<{ folders: SessionFolderRecord[]; sessionFolders: Record<string, FolderId> }> {
    await this.ensureRootEncoding()
    if (this.folderState !== undefined) return this.folderState
    try {
      const raw = JSON.parse(await readFile(join(this.root, 'folders.json'), 'utf8')) as Partial<JsonlFolderState>
      this.folderState = {
        folders: Array.isArray(raw.folders) ? raw.folders as SessionFolderRecord[] : [],
        sessionFolders: raw.sessionFolders !== null && typeof raw.sessionFolders === 'object'
          ? raw.sessionFolders as Record<string, FolderId> : {},
      }
    } catch (error: unknown) {
      if (!isENOENT(error)) throw error
      this.folderState = { folders: [], sessionFolders: {} }
    }
    return this.folderState
  }

  private async updateFolderState<T>(update: (state: { folders: SessionFolderRecord[]; sessionFolders: Record<string, FolderId> }) => T | Promise<T>): Promise<T> {
    const operation = this.folderTail.then(async () => {
      const state = await this.readFolderState()
      const result = await update(state)
      await mkdir(this.root, { recursive: true })
      const temporary = join(this.root, `.folders-${randomBytes(8).toString('hex')}.tmp`)
      await writeFile(temporary, JSON.stringify(state), { encoding: 'utf8', mode: 0o600 })
      await rename(temporary, join(this.root, 'folders.json'))
      return result
    })
    this.folderTail = operation.then(() => undefined, () => undefined)
    return operation
  }
