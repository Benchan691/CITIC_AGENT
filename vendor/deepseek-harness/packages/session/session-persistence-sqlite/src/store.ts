...
export class SqliteStore implements PersistenceBackend<number> {
  ...
  async deleteSession(id: SessionId): Promise<boolean> {
    await this.open()
    return this.mutate('delete session', () => this.db.prepare(sql('delete-session')).run(id).changes > 0)
  }

  async listFolders(): Promise<SessionFolderRecord[]> {
    await this.open()
    return this.db.prepare(sql('select-folders')).all().map((value) => {
  ...
  async createFolder(folder: SessionFolderRecord): Promise<void> {
    await this.open()
    this.mutate('create folder', () => {
      this.db.prepare(sql('insert-folder'))
        .run(folder.id, folder.name, folder.description ?? null, folder.createdAt, folder.updatedAt)
    })
  }

  async renameFolder(id: FolderId, name: string, description?: string): Promise<void> {
    await this.open()
    this.mutate('rename folder', () => {
      const result = this.db.prepare(sql('update-folder')).run(name, description ?? null, Date.now(), id)
      if (result.changes === 0) throw new Error(`folder '${id}' not found`)
    })
  }

  async deleteFolder(id: FolderId): Promise<boolean> {
    await this.open()
    return this.mutate('delete folder', () => this.db.prepare(sql('delete-folder')).run(id).changes > 0)
  }

  async setSessionFolder(id: SessionId, folderId: FolderId): Promise<void> {
    await this.open()
    this.mutate('set session folder', () => {
      if (this.db.prepare(sql('select-folder-exists')).get(folderId) === undefined) {
        throw new Error(`folder '${folderId}' not found`)
      }
      if (this.db.prepare(sql('set-session-folder')).run(folderId, id).changes === 0) {
  ...
  private mutate<T>(operation: string, mutation: () => T): T {
    this.db.exec(sql('begin-immediate'))
    try {
      validateSchemaForMutation(this.databaseConstructor, this.db, this.databasePath)
