...
function configureDatabase(
  ...
  try {
    ...
    if (onDisk !== 0 && onDisk !== 16 && onDisk !== 17 && onDisk !== SCHEMA_VERSION) {
      throw new Error(
    ...
    else if (onDisk === 16) db.exec(sql('migrate-16-to-18'))
    else if (onDisk === 17) db.exec(sql('migrate-17-to-18'))
...
export function decodeSessionRow(value: unknown): SessionRow {
  ...
  return {
    ...
    folder_id: nullableStringField(row, 'folder_id'),
  }
}
...
export function rowToMeta(row: SessionRow): SessionHeader {
  return {
    ...
    ...row.folder_id === null ? {} : { folderId: FolderId(row.folder_id) },
  }
}
