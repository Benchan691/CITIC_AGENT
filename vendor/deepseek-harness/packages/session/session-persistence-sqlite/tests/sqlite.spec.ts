...
describe('SessionPersistenceSqlite physical packing', () => {
  ...
  it('rejects an older SQLite physical schema', async () => {
    ...
    seed.exec(testSql('set-user-version-15'))
    seed.close()
    await chmod(path, 0o600)
    await expect(openDatabase(DatabaseSync, path, 'wal', DEFAULT_BUSY_TIMEOUT_MS))
      .rejects.toThrow(/schema version 15.*incompatible/)
  })

  it('migrates upstream schema 17 to logical-folder schema 18', async () => {
    const path = await freshDbPath('dsh-sqlite-schema-17-')
    const legacy = new DatabaseSync(path)
    legacy.exec(testSql('schema-17'))
    legacy.close()
    await chmod(path, 0o600)
    ...
    expect(migrated.prepare('SELECT id, folder_id FROM sessions').get()).toEqual({ id: 'v17-session', folder_id: null })
    migrated.close()
  })

  it('migrates SOC schema 16 without losing folders, sessions, or events', async () => {
    const path = await freshDbPath('dsh-sqlite-schema-16-')
    const legacy = new DatabaseSync(path)
    legacy.exec(testSql('schema-16-with-folder'))
    legacy.close()
    await chmod(path, 0o600)

    const migrated = await openDatabase(DatabaseSync, path, 'wal', DEFAULT_BUSY_TIMEOUT_MS)
    expect(migrated.prepare(testSql('select-user-version')).get()).toEqual({ user_version: SCHEMA_VERSION })
    expect(migrated.prepare('SELECT id, folder_id FROM sessions').get()).toEqual({ id: 'v16-session', folder_id: 'folder-16' })
    expect(migrated.prepare('SELECT id, name FROM folders').get()).toEqual({ id: 'folder-16', name: 'Legacy folder' })
    expect(migrated.prepare('SELECT session_id, seq, type FROM events').get())
      .toEqual({ session_id: 'v16-session', seq: 0, type: 'turn/start' })
    migrated.close()
  })
...
describe('SessionPersistenceSqlite schema ownership', () => {
  ...
  it('rejects unversioned, incompatible, and foreign-application databases', async () => {
    ...
    incompatible.exec(testSql('set-user-version-15'))
    incompatible.close()
