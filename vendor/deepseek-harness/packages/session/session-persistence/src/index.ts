...
import type { FolderId, SessionEvent, SessionId, SessionHeader } from '@deepseek-ai/dsh-session'
...
/** Durable logical chat folder record. */
...
export abstract class SessionPersistence extends Service {
  ...
  /**
   * Delete one durable session log, including its persisted events/messages.
   * Backends must make this operation idempotent and return whether a durable
   * record was removed. The default is a loud failure so a provider cannot
   * silently report cleanup success while retaining messages.
   * @param _id - the durable session id to remove.
   * @returns true when a durable record was removed.
   */
  async deleteSession(_id: SessionId): Promise<boolean> {
    throw new Error('session persistence backend does not support deleteSession')
  }

  /** List logical folders, independent from filesystem cwd. */
  async listFolders(): Promise<SessionFolderRecord[]> {
    throw new Error('session persistence backend does not support folders')
  }

  /** Persist one new logical folder. */
  async createFolder(_folder: SessionFolderRecord): Promise<void> {
    throw new Error('session persistence backend does not support folders')
  }

  /** Rename one logical folder. */
  async renameFolder(_id: FolderId, _name: string, _description?: string): Promise<void> {
    throw new Error('session persistence backend does not support folders')
  }

  /** Delete one logical folder after its sessions have been removed. */
  async deleteFolder(_id: FolderId): Promise<boolean> {
    throw new Error('session persistence backend does not support folders')
  }

  /** Set explicit folder ownership for one persisted session. */
  async setSessionFolder(_id: SessionId, _folderId: FolderId): Promise<void> {
    throw new Error('session persistence backend does not support folders')
  }
