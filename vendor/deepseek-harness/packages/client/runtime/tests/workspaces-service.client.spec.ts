...
describe('WorkspaceManager', () => {
  it('keeps the durable archive baseline while grouping by logical folders', async () => {
    const api = new FakeApiClient()
    Object.assign(api, {
      folders: {
        list: () => Promise.resolve(ok({ items: [{
          folderId: 'folder-one', name: 'Folder one', sessionIds: [sid('visible')],
          createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z',
        }] })),
      },
    })
    api.onWorkspaceList = () => Promise.resolve(ok({
      items: [], archivedSessionIds: [sid('archived')],
    }) as never)

    const manager = new WorkspaceManager(api)
    await manager.refresh()

    expect(manager.getSnapshot().items).toMatchObject([{
      workspaceId: 'folder-one', title: 'Folder one', sessionIds: ['visible'],
    }])
    expect(manager.getSnapshot().archivedSessionIds).toEqual(['archived'])
  })
...
describe('WorkspaceRuntime', () => {
  ...
  it('moves the current blank session when a logical folder is chosen', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    let moved = false
    const moveSessionToFolder = vi.fn(async (payload: { folderId: string; sessionId: SessionId }) => {
      moved = true
      return ok({ folder: {
        folderId: payload.folderId, name: 'Test', sessionIds: [payload.sessionId],
        createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z',
      } })
    })
    Object.assign(api, { folders: {
      list: () => Promise.resolve(ok({ items: [{
        folderId: 'test', name: 'Test', sessionIds: moved ? [sid('blank')] : [],
        createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z',
      }] })),
      moveSessionToFolder,
    } })
    ...
    await expect(workspaces.connectWorkspace(wid('test'))).resolves.toBe('blank')
    await Promise.resolve()
    expect(moveSessionToFolder).toHaveBeenCalledWith({ folderId: 'test', sessionId: 'blank' })
    expect(api.callsOf('session.create')).toEqual([])
    expect(sessions.list.getSnapshot().byId[sid('blank')]?.folderId).toBe('test')
  })
  ...
  it('returns created Workspaces and preserves Host business errors', async () => {
    ...
    await expect(rejected).rejects.toThrow('missing')
  ...
  it('targets New Session at explicit, current-session, then recent Workspaces and creates an unscoped chat with none', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    ...
    emptyApi.onCreate = () => Promise.resolve(ok({ sessionId: sid('unscoped') }))
    emptyWorkspaces.startSession()
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(emptyApi.callsOf('session.create')).toEqual([{}])
  })

  it('does not let a slow New Session replace a newer user selection', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    api.onList = () => Promise.resolve(ok({ items: [
      { sessionId: sid('current'), updatedAt: 2, running: false, blank: false },
      { sessionId: sid('chosen'), updatedAt: 1, running: false, blank: true },
    ] as never[] }))
    await sessions.refresh()
    sessions.open(sid('current'))
    const pending = deferred<SessionId>()
    vi.spyOn(workspaces, 'connectWorkspace').mockReturnValue(pending.promise)

    workspaces.startSession(wid('general'))
    sessions.open(sid('chosen'))
    pending.resolve(sid('late'))
    await Promise.resolve()

    expect(sessions.list.getSnapshot().current).toBe('chosen')
  })
...
describe('startInitialSelection', () => {
  ...
  it('stays idle when a session is already current and creates an unscoped session on first launch', async () => {
    ...
    noRecent.api.onCreate = () => Promise.resolve(ok({ sessionId: sid('first-chat') }))
    ...
    expect(noRecent.sessions.list.getSnapshot().current).toBe('first-chat')
    expect(() => noRecent.workspaces.startInitialSelection()).toThrow(/already started/)
    stopEmpty()
  })

  it('opens the most recent persisted session without recursively selecting it', async () => {
    const b = bench()
    b.api.onList = () => Promise.resolve(ok({ items: [
      { sessionId: sid('older'), updatedAt: 1, running: false, blank: false },
      { sessionId: sid('newer'), updatedAt: 2, running: false, blank: false },
    ] as never[] }))
    const stop = b.workspaces.startInitialSelection()
    await b.workspaces.refresh()
    await b.sessions.refresh()

    expect(b.sessions.list.getSnapshot().current).toBe('newer')
