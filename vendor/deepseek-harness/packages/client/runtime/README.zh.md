...
`WorkspaceRuntime.connectWorkspace(folderId)` 解析 New Session 流程最终落入的会话。当前 Session 为空时，选择逻辑 Folder 会移动该 Session 并刷新两份投影，从而保留 composer 与草稿；否则先复用该 Folder 的既有空 Session，未命中再调用 `session.create({folderId})`。共享的 `startSession` 操作优先使用明确指定的 Folder，其次使用当前 Session 所属 Folder，再其次使用派生的最近活跃 Folder；没有 Folder 时创建不受 Folder 约束的 Session。每次明确导航都会淘汰更早仍在等待的 New Session 结果，初始选择则在打开既有 Session 前先把启动流程标记为完成。`SessionSummary.blank` 镜像主机派生的空日志位，在客户端只降不升：由 `session.list`／`host/session-added` 帧播种，本地首次获 Host 接受的 `prompt()` 与任何 `running: true` 状态帧翻为 false，每次列表重拉重新对齐。列表界面隐藏 blank 行；store 保留全部行。`SessionRuntime.create` 接受可选的、由调用方预先分配的 SessionId，失败时抛出 `SessionCreateError`（携带 `requestedSessionId`）。
