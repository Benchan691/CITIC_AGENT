...
import type {
  ...
  SessionSummary, SubagentAddress, SubagentCatalog, JobView, WorkspaceId, FolderId,
...
export class SessionManager {
   ...
   */
  async create(
    opts: { workspaceId?: WorkspaceId; folderId?: FolderId; cwd?: string; sessionId?: SessionId } = {},
  ): Promise<RpcResult<{ sessionId: SessionId }>> {
    try {
      const shared = opts.sessionId === undefined ? {} : { sessionId: opts.sessionId }
      const payload = opts.folderId !== undefined
        ? { folderId: opts.folderId, ...shared }
    ...
    switch (frame.type) {
      case 'host/session-added': {
        this.mergeSummary({
          ...
          ...(frame.folderId !== undefined ? { folderId: frame.folderId } : {}),
  ...
  switch (mutation.kind) {
    case 'upsert': {
      ...
      const filled: SessionSummary = {
        ...
        // Folder membership is mutable: a fresh list row after a move must
        // replace the former owner instead of preserving the first folder.
        ...(mutation.summary.folderId !== undefined ? { folderId: mutation.summary.folderId } : {}),
      ...
      if (filled.cwd === existing.cwd && filled.folderId === existing.folderId
