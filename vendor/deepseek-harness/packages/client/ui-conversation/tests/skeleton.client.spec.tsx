...
function mount(
  ...
  options: {
    ...
    /** Logical folder id carried by the session summary. */
  ...
  const childRow = {
    ...
    ...(options.sessionFolderId === undefined ? {} : { folderId: options.sessionFolderId as never }),
  }
...
describe('ConversationRoot resident composer', () => {
  ...
  it('keeps a session-driven composer available without a selected folder', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    fireEvent.click(b.view.getByRole('button', { name: '选择文件夹（可选）' }))
  ...
  it('uses the session folder id when the folder session list is stale', () => {
    const b = mount(
      conversationSnapshot({ composerPhase: 'blank', blank: true }),
      [
        { ...workspace('one'), sessionIds: [] },
        { ...workspace('second'), title: 'Selected Folder', sessionIds: [] },
      ],
      undefined,
      { summaryBlank: true, sessionFolderId: 'second' },
    )
    expect(b.view.getByText('Selected Folder')).toBeTruthy()
  })
  ...
  it('blank session keeps the interactive picker chip (workspace switchable until the first message)', () => {
    const b = mount(conversationSnapshot({ composerPhase: 'blank', blank: true }))
    const chip = b.view.getByRole('button', { name: '选择文件夹（可选）' })
