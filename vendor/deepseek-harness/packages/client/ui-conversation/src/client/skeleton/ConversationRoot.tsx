...
export function ConversationRoot({
  ...
  const sessionFolderId = useSessions(s => sessionId === undefined ? undefined : s.byId[sessionId]?.folderId)
  ...
  const sessionWorkspace = sessionId === undefined
    ...
    : workspaces.items.find(workspace =>
      (sessionFolderId !== undefined
        && workspace.workspaceId === sessionFolderId as unknown as WorkspaceId)
  ...
  // Clear the pending pick once the session lands in it. A folder list can
  // briefly be stale while session.create/session.open completes, so the
  // absence of a folder from that projection is not evidence that it was
  // deleted; selectWorkspace's rejection path handles real failures.
  useEffect(() => {
  ...
  // A chat session is the only prerequisite for the composer. The folder chip
  // is optional organization state and must never turn a valid blank session
  // into a disabled workspace gate.
  ...
  return (
    <div className={css.root} data-phase={phase} data-wallpaper-surface="conversation">
