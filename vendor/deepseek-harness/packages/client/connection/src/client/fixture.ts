...
import type {
  ...
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView, FolderId, FolderView,
} from './api.ts'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const sessions: SessionSummary[] = options.empty ? [] : [
    { sessionId: sid('fx-alpha'), updatedAt: Date.now(), running: true, blank: false, cwd: '/tmp/fixture', folderId: 'fx-ws-fixture' as FolderId },
    { sessionId: sid('fx-beta'), updatedAt: Date.now() - 60_000, running: false, blank: false, parentSessionId: sid('fx-alpha'), cwd: '/tmp/fixture', folderId: 'fx-ws-fixture' as FolderId },
    { sessionId: sid('fx-gamma'), updatedAt: Date.now() - 120_000, running: false, blank: false, cwd: '/tmp/fixture', folderId: 'fx-ws-fixture' as FolderId },
  ]
  ...
  const asFolder = (workspace: WorkspaceView): FolderView => ({
    folderId: workspace.workspaceId as unknown as FolderId,
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      delete: (request) => {
        ...
        if (index === -1) return err(request, { code: 'session-not-found', message: `no session ${request.payload.sessionId}`, details: { sessionId: request.payload.sessionId } })
        sessions.splice(index, 1)
        logs.delete(request.payload.sessionId)
        for (const workspace of workspaces) workspace.sessionIds = workspace.sessionIds.filter(id => id !== request.payload.sessionId)
        return ok(request, { deleted: true as const })
      },
      create: async (request) => {
        const workspace = request.payload.folderId !== undefined
          ? workspaces.find(w => w.workspaceId === request.payload.folderId as unknown as WorkspaceId)
          : request.payload.workspaceId === undefined
            ? undefined
            : workspaces.find(w => w.workspaceId === request.payload.workspaceId)
        if (request.payload.folderId !== undefined && workspace === undefined) {
          return err(request, { code: 'folder-not-found', message: `no folder ${request.payload.folderId}`, details: { folderId: request.payload.folderId } })
        }
        ...
        const cwd = request.payload.folderId !== undefined ? '/tmp/fixture/.state/runtime' : workspace?.path ?? request.payload.cwd ?? '/tmp/fixture'
        ...
        const created: SessionSummary = {
          ...
          ...(request.payload.folderId === undefined ? {} : { folderId: request.payload.folderId }),
        }
    ...
    folders: {
      list: request => ok(request, { items: workspaces.map(asFolder) }),
      get: (request) => {
        const folder = workspaces.find(candidate => candidate.workspaceId === request.payload.folderId as unknown as WorkspaceId)
        return folder === undefined
          ? err(request, { code: 'folder-not-found', message: `no folder ${request.payload.folderId}`, details: { folderId: request.payload.folderId } })
          : ok(request, { folder: asFolder(folder) })
      },
      create: (request) => {
        const name = request.payload.name.trim()
        if (name.length === 0) return err(request, { code: 'folder-invalid-name', message: 'folder name must not be empty', details: { name: request.payload.name } })
        if (workspaces.some(folder => folder.title === name)) return err(request, { code: 'folder-name-conflict', message: `folder name '${name}' is already in use`, details: { name } })
        const now = new Date().toISOString()
        const created: WorkspaceView = { workspaceId: wid(`fx-ws-${nextWorkspace++}`), path: '', title: name, sessionIds: [], createdAt: now, updatedAt: now }
        workspaces.unshift(created)
        return ok(request, { folder: asFolder(created) })
      },
      rename: (request) => {
        const folder = workspaces.find(candidate => candidate.workspaceId === request.payload.folderId as unknown as WorkspaceId)
        if (folder === undefined) return err(request, { code: 'folder-not-found', message: `no folder ${request.payload.folderId}`, details: { folderId: request.payload.folderId } })
        const name = request.payload.name.trim()
        if (name.length === 0) return err(request, { code: 'folder-invalid-name', message: 'folder name must not be empty', details: { name: request.payload.name } })
        if (workspaces.some(candidate => candidate !== folder && candidate.title === name)) return err(request, { code: 'folder-name-conflict', message: `folder name '${name}' is already in use`, details: { name } })
        folder.title = name
        folder.updatedAt = new Date().toISOString()
        return ok(request, { folder: asFolder(folder) })
      },
      delete: (request) => {
        const index = workspaces.findIndex(candidate => candidate.workspaceId === request.payload.folderId as unknown as WorkspaceId)
        if (index === -1) return err(request, { code: 'folder-not-found', message: `no folder ${request.payload.folderId}`, details: { folderId: request.payload.folderId } })
        const folder = workspaces[index]
        if (folder === undefined) return err(request, { code: 'folder-not-found', message: `no folder ${request.payload.folderId}`, details: { folderId: request.payload.folderId } })
        const ids = [...folder.sessionIds]
        for (const id of ids) {
          const sessionIndex = sessions.findIndex(session => session.sessionId === id)
          if (sessionIndex !== -1) sessions.splice(sessionIndex, 1)
          logs.delete(id)
        }
        workspaces.splice(index, 1)
        return ok(request, { deleted: true as const, deletedSessionCount: ids.length })
      },
      moveSessionToFolder: (request) => {
        const target = workspaces.find(candidate => candidate.workspaceId === request.payload.folderId as unknown as WorkspaceId)
        if (target === undefined) return err(request, { code: 'folder-not-found', message: `no folder ${request.payload.folderId}`, details: { folderId: request.payload.folderId } })
        if (summaryOf(request.payload.sessionId) === undefined) return err(request, { code: 'session-not-found', message: `no session ${request.payload.sessionId}`, details: { sessionId: request.payload.sessionId } })
        for (const folder of workspaces) folder.sessionIds = folder.sessionIds.filter(id => id !== request.payload.sessionId)
        target.sessionIds.unshift(request.payload.sessionId)
        const summary = summaryOf(request.payload.sessionId)
        if (summary !== undefined) summary.folderId = request.payload.folderId
        return ok(request, { folder: asFolder(target) })
      },
      listSessionsByFolder: (request) => {
        const folder = workspaces.find(candidate => candidate.workspaceId === request.payload.folderId as unknown as WorkspaceId)
        if (folder === undefined) return err(request, { code: 'folder-not-found', message: `no folder ${request.payload.folderId}`, details: { folderId: request.payload.folderId } })
        return ok(request, { items: sessions.filter(session => folder.sessionIds.includes(session.sessionId)) })
      },
    },
    agentPresets: {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'folder.list': return this.api.folders.list(request)
      case 'folder.get': return this.api.folders.get(request)
      case 'folder.create': return this.api.folders.create(request)
      case 'folder.rename': return this.api.folders.rename(request)
      case 'folder.delete': return this.api.folders.delete(request)
      case 'folder.moveSessionToFolder': return this.api.folders.moveSessionToFolder(request)
      case 'folder.listSessionsByFolder': return this.api.folders.listSessionsByFolder(request)
