...
目录选择是可选能力。未组合 `ctx.directoryPicker` 时，网关仍会启动，只有三个旧目录方法返回 `directory-picker-unavailable`，其 capability 为 `none`。组合后，`native` 提供 `host.pickDirectory`，`browse` 提供 `host.listDirectory` 与 `host.createDirectory`；调用另一能力 kind 时返回同一类型化错误。浏览器载体的前缀级信任栅栏像覆盖其他所有 `/api` 请求一样覆盖这些方法。
