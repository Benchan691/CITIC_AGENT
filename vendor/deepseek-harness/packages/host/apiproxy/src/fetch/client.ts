...
import {
  folderCreateValueSchema, folderDeleteValueSchema, folderGetValueSchema,
  folderListSessionsValueSchema, folderListValueSchema, folderMoveSessionValueSchema, folderRenameValueSchema,
} from '../api/folders.schema.ts'
import {
...
export interface IApiClient {
  ...
  folders?: {
    ...
    get(payload: RequestPayload<'folder.get'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'folder.get'>>>
    ...
    moveSessionToFolder(payload: RequestPayload<'folder.moveSessionToFolder'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'folder.moveSessionToFolder'>>>
    listSessionsByFolder(payload: RequestPayload<'folder.listSessionsByFolder'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'folder.listSessionsByFolder'>>>
  }
  skills: {
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'folder.get': folderGetValueSchema,
  'folder.create': folderCreateValueSchema,
  'folder.rename': folderRenameValueSchema,
  'folder.delete': folderDeleteValueSchema,
  'folder.moveSessionToFolder': folderMoveSessionValueSchema,
  'folder.listSessionsByFolder': folderListSessionsValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected async callUnary<K extends keyof RpcMethodMap>(
    ...
    let value: ResponseValue<K>
    try {
      // Keep the raw application value beside the parse failure. This is
      // diagnostic-only; callers still receive a folded transport error and
      // never a schema implementation detail in normal UI copy.
      value = UNARY_VALUE_SCHEMAS[method].parse(full.result.value) as ResponseValue<K>
    } catch (error) {
      console.error(`[client-connection] invalid ${method} response value`, {
        raw: full.result.value,
        error,
      })
      throw error
    }
  ...
  readonly folders: NonNullable<IApiClient['folders']> = {
    list: (payload, signal) => this.callUnary('folder.list', payload, signal),
    get: (payload, signal) => this.callUnary('folder.get', payload, signal),
    ...
    moveSessionToFolder: (payload, signal) => this.callUnary('folder.moveSessionToFolder', payload, signal),
    listSessionsByFolder: (payload, signal) => this.callUnary('folder.listSessionsByFolder', payload, signal),
  }
