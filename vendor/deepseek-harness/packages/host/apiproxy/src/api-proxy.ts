...
// Type-only: resolves the optional post-delete cleanup service without making
// the gateway depend on a particular profile's plugin composition.
import type {} from '@deepseek-ai/dsh-workspace-session-cleanup'
import type { FolderView as NativeFolderView } from '@deepseek-ai/dsh-session-folders'
import type {} from '@deepseek-ai/dsh-session-folders'
import {
...
function sessionListFields(header: SessionHeader, events: readonly SessionEvent[] = []): {
  ...
  return {
    ...
    ...header.folderId === undefined ? {} : { folderId: header.folderId },
...
function folderView(folder: NativeFolderView): FolderView {
  return {
    folderId: folder.id,
    name: folder.name,
...
/** Error classes can be duplicated when plugins are bundled separately. */
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // Logical folders deliberately do not become project directories. Harness
  // still needs a cwd for tool/runtime compatibility, so folder-created
  // sessions share this internal application-owned sandbox.
  const runtimeCwd = join(defaults.cwd, '.state', 'runtime')
  ...
  async function ensureSession(
    ...
    presetId?: string,
    folderId?: SessionHeader['folderId'],
  ): Promise<Agent> {
  ...
  return {
    sessions: {
      ...
      async create(request) {
        ...
        const folderStore = ctx.get('sessionFolders')
        const folderId = request.payload.folderId
        if (folderId !== undefined && folderStore === undefined) {
          return err(request, {
            code: 'internal',
            message: 'logical folders are unavailable: this deployment does not mount @deepseek-ai/dsh-session-folders',
            details: {},
          })
        }
        if (folderId !== undefined && await folderStore?.getFolder(folderId) === undefined) {
        ...
        const cwd = folderId === undefined
          ? workspace?.path ?? request.payload.cwd ?? defaults.cwd
          : runtimeCwd
        const requestedPreset = request.payload.agentPreset
        try {
          if (folderId !== undefined) await mkdir(runtimeCwd, { recursive: true })
          await ensureSession(
            sessionId,
            cwd,
        ...
        if (folderId !== undefined && folderStore !== undefined) {
          try {
            await folderStore.moveSessionToFolder(sessionId, folderId)
          } catch (error: unknown) {
            return err(request, {
              code: 'internal',
              message: `session "${sessionId}" was created but could not be assigned to folder "${folderId}": ${String(error)}`,
              details: {},
            })
          }
        }
      ...
      async delete(request) {
        ...
        try {
          const folderStore = ctx.get('sessionFolders')
          const deleted = folderStore === undefined
            ? await (async () => {
              const live = ctx.sessions.get(sessionId)
              if (live !== undefined) {
                const agent = ctx.agents.get(sessionId)
                if (agent !== undefined) await agent.ctx.fiber.dispose()
                const remaining = ctx.sessions.get(sessionId)
                if (remaining !== undefined) await ctx.sessions.flush(remaining)
                ctx.sessions.deleteSession(sessionId)
              }
              return (await ctx.get('sessionPersistence')?.deleteSession(sessionId)) === true
            })()
    ...
    folders: {
      async list(request) {
        const folders = ctx.get('sessionFolders')
        if (folders === undefined) {
          return err(request, { code: 'internal', message: 'logical folders are unavailable', details: {} })
        }
        return ok(request, { items: (await folders.listFolders()).map(folderView) })
      },

      async get(request) {
        const folders = ctx.get('sessionFolders')
        if (folders === undefined) return err(request, { code: 'internal', message: 'logical folders are unavailable', details: {} })
        const folder = await folders.getFolder(request.payload.folderId)
        return folder === undefined
          ? folderNotFound(request, request.payload.folderId)
          : ok(request, { folder: folderView(folder) })
      },
      ...
      async create(request) {
        ...
        if (folders === undefined) return err(request, { code: 'internal', message: 'logical folders are unavailable', details: {} })
        try {
          return ok(request, { folder: folderView(await folders.createFolder(request.payload.name, request.payload.description)) })
        } catch (error: unknown) {
          const message = error instanceof Error ? error.message : String(error)
          if (errorNamed(error, 'FolderInvalidNameError')) {
            return err(request, { code: 'folder-invalid-name', message, details: { name: request.payload.name } })
          }
          if (errorNamed(error, 'FolderNameConflictError')) {
            return err(request, { code: 'folder-name-conflict', message, details: { name: request.payload.name.trim() } })
          }
          return err(request, {
            code: 'internal',
            message,
      ...
      async rename(request) {
        ...
        try {
          return ok(request, {
            folder: folderView(await folders.renameFolder(
          ...
          if (errorNamed(error, 'FolderNotFoundError')) return folderNotFound(request, request.payload.folderId)
          const message = error instanceof Error ? error.message : String(error)
          if (errorNamed(error, 'FolderInvalidNameError')) {
            return err(request, { code: 'folder-invalid-name', message, details: { name: request.payload.name } })
          }
      ...
      async delete(request) {
        ...
        if (folders === undefined) return err(request, { code: 'internal', message: 'logical folders are unavailable', details: {} })
        try {
          const deletedSessionCount = await folders.deleteFolder(request.payload.folderId)
          return ok(request, { deleted: true as const, deletedSessionCount })
        } catch (error: unknown) {
          if (errorNamed(error, 'FolderNotFoundError')) return folderNotFound(request, request.payload.folderId)
          return err(request, {
            code: 'internal',
            message: `failed to delete folder "${request.payload.folderId}": ${String(error)}`,
            details: {},
          })
        }
      },

      async moveSessionToFolder(request) {
        const folders = ctx.get('sessionFolders')
        if (folders === undefined) return err(request, { code: 'internal', message: 'logical folders are unavailable', details: {} })
        try {
          await folders.moveSessionToFolder(request.payload.sessionId, request.payload.folderId)
          const folder = await folders.getFolder(request.payload.folderId)
          if (folder === undefined) return folderNotFound(request, request.payload.folderId)
          return ok(request, { folder: folderView(folder) })
        } catch (error: unknown) {
          if (errorNamed(error, 'FolderNotFoundError')) return folderNotFound(request, request.payload.folderId)
          return err(request, { code: 'internal', message: `failed to move session "${request.payload.sessionId}": ${String(error)}`, details: {} })
        }
      },

      async listSessionsByFolder(request) {
        const folders = ctx.get('sessionFolders')
        if (folders === undefined) return err(request, { code: 'internal', message: 'logical folders are unavailable', details: {} })
        try {
          const ids = new Set(await folders.listSessionsByFolder(request.payload.folderId))
          const items = (await listVisibleSessionSummaries()).filter(item => ids.has(item.sessionId))
          return ok(request, { items })
        } catch (error: unknown) {
          if (errorNamed(error, 'FolderNotFoundError')) return folderNotFound(request, request.payload.folderId)
          return err(request, { code: 'internal', message: `failed to list folder sessions: ${String(error)}`, details: {} })
        }
      },
    },
    ...
    workspace: {
      ...
      async delete(request) {
        ...
        const operation = workspaceCreationChain.then(async () => {
          ...
          // The registry deletion is the commit boundary. Cleanup is strictly
          // after it, so a failed workspace deletion never removes history.
          await ctx.get('workspaceSessionCleanup')?.deleteWorkspaceSessions(workspace.path)
          return true
        })
        workspaceCreationChain = operation.then(() => undefined, () => undefined)
    ...
    host: {
      ...
      async pickDirectory(request, signal) {
        ...
        if (capability === undefined) {
          return err(request, {
            ...
            message: 'host.pickDirectory is unavailable in this deployment',
      ...
      async listDirectory(request, signal) {
        ...
        if (capability === undefined) {
          return err(request, {
            code: 'directory-picker-unavailable',
            message: 'host.listDirectory is unavailable in this deployment',
            details: { capability: 'none' },
          })
        }
      ...
      async createDirectory(request) {
        ...
        if (capability === undefined) {
          return err(request, {
            code: 'directory-picker-unavailable',
            message: 'host.createDirectory is unavailable in this deployment',
            details: { capability: 'none' },
          })
        }
        if (capability.kind !== 'browse') {
