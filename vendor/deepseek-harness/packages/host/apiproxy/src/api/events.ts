 ...
 * origin, project cwd, optional logical folder id, and blank bit (the
 * list-summary fields a client cannot wait for a refresh to learn); the frame
 * fires at session/created, so blank is constantly true — clients flip it on
 * the session's first `host/session-status(running:true)` (a blank session
 * never runs), and a reconnecting client takes `session.list`'s summary.blank
 * as authoritative.
