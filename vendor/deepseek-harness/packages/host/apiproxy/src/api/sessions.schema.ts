...
export const sessionCreateRequestSchema = z.object({
  ...
  payload => [payload.workspaceId, payload.folderId, payload.cwd].filter(value => value !== undefined).length <= 1,
  { message: 'session.create accepts one of workspaceId, folderId, or cwd' },
...
/** session.delete request/value. */
export const sessionDeleteRequestSchema = z.object({ sessionId: sessionIdSchema })
export const sessionDeleteValueSchema = z.object({ deleted: z.literal(true) })
