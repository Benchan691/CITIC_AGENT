...
import {
  contentBlockSchema, folderIdSchema, messageIdSchema, sessionEventSchema, sessionIdSchema, toolEventViewSchema,
...
export const hostFrameSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('host/session-status'), sessionId: sessionIdSchema, running: z.boolean(), lastSeq: z.number().int().min(-1) }),
