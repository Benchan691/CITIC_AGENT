...
async function harness(
  ...
  extras: {
    ...
    canOpenPath?: () => boolean
    workspaceSessionCleanup?: { deleteWorkspaceSessions: (path: string) => Promise<number> }
  } = {},
) {
  ...
  if (extras.workspaceSessionCleanup !== undefined) {
    ctx.provide('workspaceSessionCleanup', extras.workspaceSessionCleanup as never)
  }
...
describe('host.pickDirectory', () => {
  it('keeps the gateway usable when no legacy picker is composed', async () => {
...
describe('Host Workspace increments', () => {
  ...
  it('deletes the registration and keeps sessions when cleanup is not mounted', async () => {
    const { api, ctx, root } = await harness()
  ...
  it('runs workspace session cleanup only after the registry deletion commits', async () => {
    const cleanup = vi.fn(async () => 1)
    const { api, root } = await harness(undefined, undefined, {
      workspaceSessionCleanup: { deleteWorkspaceSessions: cleanup },
    })
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'cleanup-after-delete') }))).workspace

    expectOk(await api.workspace.delete(request({ workspaceId: workspace.workspaceId })))
    expect(cleanup).toHaveBeenCalledWith(workspace.path)

    const secondCleanup = vi.fn(async () => 1)
    const failed = await harness(undefined, undefined, {
      workspaceSessionCleanup: { deleteWorkspaceSessions: secondCleanup },
    })
    const failedWorkspace = expectOk(await failed.api.workspace.create(request({ path: stageDir(failed.root, 'cleanup-not-run') }))).workspace
    vi.spyOn(failed.ctx.workspaceRegistry, 'delete').mockRejectedValueOnce(new Error('workspace store unavailable'))
    await expect(failed.api.workspace.delete(request({ workspaceId: failedWorkspace.workspaceId }))).rejects.toThrow('workspace store unavailable')
