...
function fakeApi(overrides: Partial<{ muxFrames: MuxFrame[]; hostFrames: HostFrame[]; crashOn: string }> = {}): ApiProxy {
  ...
  return {
    ...
    folders: {
      async list(request) { return { rpcId: request.rpcId, result: { ok: true, value: { items: [] } } } },
      async get(request) {
        const folder = { folderId: request.payload.folderId, name: 'General', createdAt: '', updatedAt: '', sessionIds: [] }
        return { rpcId: request.rpcId, result: { ok: true, value: { folder } } }
      },
      async create(request) { return { rpcId: request.rpcId, result: { ok: true, value: { folder: { folderId: 'created' as never, name: request.payload.name, createdAt: '', updatedAt: '', sessionIds: [] } } } } },
      async rename(request) { return { rpcId: request.rpcId, result: { ok: true, value: { folder: { folderId: request.payload.folderId, name: request.payload.name, createdAt: '', updatedAt: '', sessionIds: [] } } } } },
      async delete(request) {
        const value = { deleted: true as const, deletedSessionCount: 0 }
        return { rpcId: request.rpcId, result: { ok: true, value } }
      },
      async moveSessionToFolder(request) { return { rpcId: request.rpcId, result: { ok: true, value: { folder: { folderId: request.payload.folderId, name: 'General', createdAt: '', updatedAt: '', sessionIds: [request.payload.sessionId] } } } } },
      async listSessionsByFolder(request) { return { rpcId: request.rpcId, result: { ok: true, value: { items: [] } } } },
    },
    subagents: {
      async list(request) {
...
describe('unary round trip (handler ⇄ client, no network)', () => {
  ...
  it('covers create/prompt/updateQueue/cancel/describe passthrough', async () => {
    ...
    expect((await c.folders.create({ name: 'SOC Agent' })).result).toMatchObject({
      ok: true, value: { folder: { folderId: 'created', name: 'SOC Agent' } },
    })
  })
