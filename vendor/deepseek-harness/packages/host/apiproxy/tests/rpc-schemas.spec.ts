...
import { folderCreateValueSchema } from '../src/api/folders.schema.ts'
...
describe('rpcErrorSchema', () => {
  it('accepts every code branch with its required details', () => {
    ...
    expect(rpcErrorSchema.parse({ code: 'workspace-protected', message: 'm', details: { workspaceId: 'w' } }).code).toBe('workspace-protected')
...
describe('folder response schemas', () => {
  it('validate the application value after the transport envelope is unwrapped', () => {
    const value = {
      folder: {
        folderId: 'folder-1', name: 'SOC Agent', createdAt: '2026-01-01', updatedAt: '2026-01-01', sessionIds: [],
      },
    }
    expect(folderCreateValueSchema.parse(value)).toEqual(value)
    expect(() => folderCreateValueSchema.parse({ ok: true, value })).toThrow()
  })
})
...
describe('sessions domain schemas', () => {
  ...
  it('validates the per-method request/value pairs', () => {
    ...
    expect(() => sessionCreateRequestSchema.parse({ workspaceId: 'w1', cwd: '/w' })).toThrow(/one of workspaceId, folderId, or cwd/)
...
describe('host domain schemas', () => {
  it('validates describe request/value', () => {
    ...
    const value = hostDescribeValueSchema.parse({
      ...
      logicalFolders: false,
    })
    expect(value).toMatchObject({ provider: 'p', model: 'm', attachedSessions: 2, canOpenPath: true, logicalFolders: false })
...
describe('respond payload schemas', () => {
  it('validates approval and question answer payloads', () => {
    ...
    const remembered = approvalResponsePayloadSchema.parse({
      sessionId: 's', approvalId: 'a', outcome: 'allowed-once', remember: 'tool',
    })
    expect(remembered.remember).toBe('tool')
