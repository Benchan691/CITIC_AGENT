...
describe('web e2e: navigation & panes over a rich seeded session', () => {
  ...
  beforeEach(async () => {
    ...
    await page.getByRole('button', { name: 'Search sessions' }).waitFor({ timeout: 30_000 })
  }, 120_000)
  ...
  it.skipIf(MODE === 'record')('downloads through the Session Header and /export with one dialog', async () => {
    ...
    await observer.goto(scaffold.baseUrl, { waitUntil: 'load' })
    await observer.getByRole('button', { name: 'Search sessions' }).waitFor({ timeout: 30_000 })
