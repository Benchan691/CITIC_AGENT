...
async function persistSeedSession(
  ...
  // The scaffold is already booted when fixtures are written, so the folder
  // store's one-time startup migration cannot see them. Mirror normal session
  // creation by assigning every cold fixture to the default logical folder.
  const folders = await scaffold.ctx.sessionFolders.listFolders()
  const target = folders.find(folder => folder.name === 'General') ?? folders[0]
  if (target === undefined) throw new Error('web e2e seed has no logical folder')
  await scaffold.ctx.sessionFolders.moveSessionToFolder(meta.id, target.id)
}
