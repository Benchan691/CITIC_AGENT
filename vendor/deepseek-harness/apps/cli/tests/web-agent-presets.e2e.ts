 ...
 * agent's capabilities is the real thing, including all shipped presets.
 */
...
describe('the shipped Web composition', () => {
  ...
  it('supplies every shipped preset, and only those, from the system root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort())
      .toEqual(['citic-soc', 'code', 'cordis', 'minimal', 'standard'])
  ...
  it('composes CITIC SOC with only investigation support tools', async () => {
    const handle = await ctx.agents.create({
      sessionId: SessionId('preset-citic-soc'),
      setup: agentCtx => ctx.agentPresets.mount(agentCtx, 'citic-soc').then(() => undefined),
    })
    try {
      expect(toolNames(ctx, handle.agent)).toEqual(['ask_user_question', 'skill'])
      expect(ctx.agentPresets.serviceFor(handle.agent, 'compaction')).toBeDefined()
      expect(ctx.agentPresets.serviceFor(handle.agent, 'terminals')).toBeUndefined()
    } finally {
      await handle.dispose()
    }
  })
