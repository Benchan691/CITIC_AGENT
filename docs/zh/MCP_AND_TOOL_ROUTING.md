...
| 工具 | 27 个（Zimbra 邮件/过滤 + 订阅） | 13 个只读工具 |
...
1. **`zimbra_send_email` 不发信。** 它是一个只读分类的统一草稿构建器，支持 `action: "send" | "reply" | "forward"`。新发信默认 text；回复/转发读取认证用户邮箱中的原信，默认 HTML，并携带 `source_message_id` 与安全的 `source_message` 预览。实际投递 = 草稿视图 → 按动作显示 `window.confirm` → `send-email` 宿主 RPC → 控制通道 → `ZimbraMailService.send_email`（门控 `ZIMBRA_ALLOW_SEND`）。模型没有任何可调用的发信工具。
2. **`zimbra_use_signature_on_email` 也不发送** — 它产出合并签名后的可编辑草稿。
3. **`preview_subscription` 与 `validate_email_filter` 是只读工具** — 计算拟议变更而不写入。
4. **技能引用已移除的工具。** `detection-engineering`/`spl-writing`（及 `false-positive-analysis` 部分）提到的 `splunk_get_detection`、`splunk_compile_citic_detection`、`splunk_backtest_detection`、`splunk_validate_detection`、`splunk_search` — 其实现代码本轮已**删除**。见 [DOCUMENTATION_AUDIT.md](DOCUMENTATION_AUDIT.md)。

## 3. 路由层（谁在哪一步检查）

可交互图见英文站 [site/mcp-tooling.html](../site/mcp-tooling.html)；可编辑源: [diagrams/mcp-routing.mmd](../diagrams/mcp-routing.mmd)。

1. **注册（原始允许列表）。** `cordis.patch.yml` 给每个服务器 `allowedToolNames`：`soc_agent` 精确 27 个；桥接只暴露 `OFFICIAL_SPLUNK_TOOL_NAMES`（13）。`dsh-mcp-client` 不注册清单之外的名字（`mcp-discovery.test.js`）。
...
- **只读（`READ_ONLY_TOOLS`，29）：** `skill` + 13 个 Splunk 读 + 8 个 Zimbra 邮件读/草稿构建器 + 4 个过滤读取/校验 + 3 个订阅读。`zimbra_send_email` 位于只读列表**因为三个动作都只创建本地草稿**，不会投递。
...
- 草稿工具结果由 `EmailDraftToolview`（统一草稿工具与签名辅助工具）渲染为可编辑卡片，而非原始 JSON。
