...
| **Draft card** | `packages/soc-agent-email-draft/src/client/EmailDraftToolview.tsx` | Renders inside the tool-call block for the unified send/reply/forward draft tool. Disable the plugin to remove only these editable tool views. See §3. |
...
1. The model calls `zimbra_send_email` with `action: send | reply | forward`, or `zimbra_use_signature_on_email` → the **local draft** arrives as the tool result → the card renders an editable form (To/CC/BCC with separator-aware parsing and dedupe; subject ≤998; body ≤18 000; text/HTML body format). Reply and forward drafts retain `source_message_id` plus a safe source preview; replies may leave To blank for Reply-To/from derivation, while forwards require To. Reply/forward default to HTML and new sends default to text.
2. **Add signature** loads `list-signatures` and merges the chosen signature above/below the body in the signature's format.
3. On Send: action-specific client validation (To required except derived replies; non-empty subject for new sends) → action-specific **`window.confirm`** → RPC `send-email` with the exact action/source metadata.
