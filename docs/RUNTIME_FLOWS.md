...
> 语言 / Language: **English** · [中文版](zh/RUNTIME_FLOWS.md)
...
**Path:** harness loads the web profile → cordis loader applies `cordis.patch.yml` (disables official sidebar/workspace and coding tools, enables skills/plan/ask-user, inserts the isolated SOC surfaces and selected feature plugins) → plugins `apply(ctx)`: auth-host (routes, transport fencing, Postgres pool), host (RPC, hooks, admin page, background), the mandatory SOC client core (auth overlay, runtime service, admin fallback), isolated sidebar/workspace, optional feature plugins, bridge (connects `splunk_mcp` if configured; else logs "disabled"), `soc-agent-mcp` (spawns `uv run unified-mcp-server`; `ensureSchema` creates tables; `load_server_env` reads `.env`) → web server listens on 3080.
...
Reads (`list_subscriptions`, `get_subscription_schema`, `preview_subscription`) default to auto-run. The client authenticates the configured local administrator with `POST /login/local`, then uses the Rust API (`POST /api/subscriptions/preview`; mode `create|update`; update requires `subscription_id`). Mutations (`create_subscription`, `update_subscription`, `delete_subscription`) are in `ACTION_CATALOG` → default state `ask` → the harness approval flow surfaces them in the UI before execution. Create/update payloads use `username`, `emails`, `organization`, `local_subscription`, and profile objects; update/delete address records by URL-encoded `subscription_id`. Zimbra subscriptions retain the existing Zimbra account email, while local subscriptions accept manually supplied recipients. Client lifecycle: form login once per process, exactly one re-login on 401, ≤5 same-host redirects (no downgrades), remote error bodies withheld.
