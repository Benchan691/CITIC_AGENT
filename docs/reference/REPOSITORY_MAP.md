...
| `BACKGROUND.md` | documentation/governance | Splunk reference context and read-only `Ruleset.csv` field contract (`RuleNum_Name`, `GID=Default`, English text, severity enum). Injected as background context by `host.js` `installBackgroundRefresh`. | Active (model context) |
...
| `tool-inventory.js` | canonical source | **Single runtime-independent source of truth** for the tool surface: `OFFICIAL_SPLUNK_TOOL_NAMES` (13 raw), `TOOL_CATALOG` (25 entries: 12 Zimbra reads, 9 mutations, 3 subscription reads, 1 UI-confirmed), `SUBSCRIPTION_READ_TOOLS` (3). Imported by both `policy.js` and `splunk-bridge.js`. Header: "Draft preparation never delivers mail." |
| `policy.js` | canonical source | Derives all policy sets from the inventory: `OFFICIAL_SPLUNK_READ_TOOLS` (13 qualified), `ZIMBRA_READ_TOOLS` (12), `READ_ONLY_TOOLS` (**29** = skill + Splunk + Zimbra reads + subscription reads), `ACTION_CATALOG` (**12** mutations), `ACTION_TOOLS`, `TOOL_CATALOG` (re-export, 25), `MANAGED_TOOL_NAMES` (24), `DOMAIN_TOOLS` (**41**), `APPROVAL_TOOLS` (= mutations). |
...
| `zimbra_service.py` + `zimbra/` (11 files) | canonical source | SOAP Zimbra client (`zimbra.py`, including `zimbra_reply_message` and `zimbra_forward_message`, both HTML-by-default with text alternatives), identity-bound core (`core/service.py`, now hosting `_EmptyAccountStore`), mail service/tools (**12 tools**, incl. unified send/reply/forward drafts), filter service/tools (9 tools, fingerprint concurrency). |
...
| `auth_cli.py` | canonical source | One-shot auth commands: `login`, `logout`, `send-email` (generic action/source metadata), `list-signatures`; also the control-channel dispatch table. |
| `tests/` (10 test files + `__init__.py`, 52 tests) | first-party test | All retained-Splunk test files removed; new `test_schema.py` (3), plus unified email-action coverage. See [TEST_COVERAGE_MATRIX.md](TEST_COVERAGE_MATRIX.md). |
...
| `soc-agent-email-draft/` | optional email-draft package | editable send/reply/forward draft views and signature/send helpers. |
