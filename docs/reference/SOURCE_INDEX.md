...
| `zimbra/zimbra.py` | SOAP transport | `soap_request`, `_TokenClient`, `zimbra_login_start`/`zimbra_complete_two_factor` (2FA AuthRequests), `zimbra_login` (normal token compatibility), `_validate_zimbra_host`, `download_attachment`, `zimbra_modify_filter_rules` |
...
| `schema.py` + `migrations/*.sql` | **Versioned SQL migrations** | `apply_migrations(connection)` (advisory lock, `soc_schema_migrations` ledger, ordered `.sql` execution), standalone `main()` reading `{"uri"}` from stdin; `001_initial.sql` (base tables), `002_remove_catalog.sql` (marker-gated legacy-table drop), `003_two_factor_challenges.sql` (bounded pending analyst challenges) |
...
| `auth_cli.py` | Auth commands (and dispatch table) | `dispatch_command`: `login` (`zimbra_login_start` → normal session or encrypted 2FA challenge), `login-2fa` (second Zimbra AuthRequest → `create_user_session`), `get-2fa`, `cancel-2fa`, `logout`, `send_email`/`send-email` (generic action metadata + gate), `list-signatures`, `command_failure`, `command_runtime` |
