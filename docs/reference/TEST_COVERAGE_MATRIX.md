...
| `policy.test.js` | 4 | Exact product tool set (**29 read-only / 41 domain / 12 approval** — pinned literally); pre-execute verdicts (delegate reads, ask mutations, deny generic); SOC-mode deployment states + RPC contracts; Full access bypasses per-tool states | `host.js`, `policy.js`, `tool-inventory.js` |
...
| `soc-agent-email-draft` | 4 | Recipient parsing/dedupe/trim, unified send/reply/forward metadata, blank-To reply handling, safe source previews, action-specific confirmation, send/signature view behavior |
...
|---|---|
| Python server exposes exactly the 27 allowlisted tools | `test_server_tools.py` (`len(tools) == 27`) ↔ `skills.test.js` "soc_agent MCP allowlist…" (same 27 names) |
| Policy sets derive exactly from `tool-inventory.js` (29/41/12) | `policy.test.js` "interactive analyst policy exposes the exact product tool set" — the inventory is now the **structural** fence (policy.js and splunk-bridge.js import it) |
