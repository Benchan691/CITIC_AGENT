...
- Source under `packages/soc-agent-client/src/` is more authoritative than compiled `lib/` output and source maps. Unlike typical builds, `lib/` is tracked in Git, so document when it must be regenerated (tsdown build), which consumers read it, and how a reviewer can tell the two have drifted.
...
- The client package under `packages/soc-agent-client`, including source, UI components, action-mode behavior, draft confirmation, settings/admin UI, attachment presentation, styles, tests, build configuration (`tsdown.config.ts`), and the tracked generated `lib/` output boundary.
