...
- `vendor/deepseek-harness` is vendored upstream software. Document its integration surface and immutable release pin, but do not rewrite upstream documentation or describe SOC code as vendor content. The local seams are `apps/soc-agent/cordis.patch.yml` (the product patch over `dsh-base` + `dsh-web-app`), the root SOC workspace, and the first-party preset under `apps/soc-agent/agent-presets/citic-soc/`, which selects instruction-file candidates (`AGENTS.md`, `CLAUDE.md`, `BACKGROUND.md`) and is asserted by tests. There is no tracked pnpm patch under `patches/`.
...
- Root governance and project material: `AGENTS.md`, `README.md`, `BACKGROUND.md`, `package.json`/`pnpm-workspace.yaml`/`lefthook.yml` manifests, `.gitignore`, `setup.sh`, and `update.sh`.
- Root loose files and leftovers: classify every tracked root file and any untracked runtime directories without treating stale, ignored data as product source. Do not skip a file because it looks accidental; classify it and say why.
...
- The Python server under `apps/soc-agent/server`, including active server registration (`unified_mcp_server/server.py`), request context, authentication, account/configuration stores, PostgreSQL use, Zimbra services, email tools, attachment conversion, subscription tools, control/admin paths (`control_server.py`, `auth_cli.py`, `admin_cli.py`), and the Python tests. The server's `pyproject.toml` + `uv.lock` are authoritative; there is no root Python requirements file and no registered in-process Splunk implementation.
- The SOC browser package suite under `packages/soc-agent-*`: mandatory core (`dsh-soc-agent-client`), foundational replacements, isolated sidebar/workspace, and optional brand, admin, action-policy, attachments, email-draft, and auto-collapse packages. Cover each package's source, slots, settings, commands, tool views, styles, tests, build configuration (`tsdown.config.ts`), and tracked generated `lib/` output boundary.
...
- The retired `patches/` directory plus the application patch manifest `apps/soc-agent/cordis.patch.yml`. Explain that the former contains no tracked patch artifact; all product composition is configuration or root-workspace code, and drift is detected by the composition/isolation tests and setup checks.
- `vendor/deepseek-harness` only to the depth needed to explain its role, immutable release manifest, extension/plugin seams (cordis plugin ids, root preset, and product patch), pristine build/run lifecycle, and security boundary. Do not attribute SOC behavior to a vendor patch.
...
Explain repository layout, supported toolchain, dependency/workspace model (the independent root pnpm workspace, exact rc.2 runtime pins, and direct local profile dependencies), the server's `pyproject.toml`/`uv.lock`, canonical source versus generated output (including tracked `lib/`), build commands, local run commands, formatting/lint/hooks (`lefthook.yml` currently contains only commented-out example jobs — verify whether any Git hooks are actually active before documenting them), product composition (`cordis.patch.yml` and the retired empty `patches/` directory), adding or changing a tool, changing policy/UI, Python development, TypeScript/React development, and keeping the pristine vendor snapshot reproducible.
