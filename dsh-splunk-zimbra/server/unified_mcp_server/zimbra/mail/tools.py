...
def register_tools(server, *, get_runtime, fresh_runtime, execute, success) -> None:
    @server.tool()
    async def zimbra_list_accounts(ctx) -> dict[str, Any]:
    ...
    @server.tool()
    async def zimbra_list_folders(ctx, account_id: str = "") -> dict[str, Any]:
    ...
    @server.tool()
    async def zimbra_search_emails(ctx, query: str, limit: int = 20, account_id: str = "") -> dict[str, Any]:
    ...
    @server.tool()
    async def zimbra_get_email(ctx, message_id: str, account_id: str = "") -> dict[str, Any]:
    ...
    @server.tool()
    async def zimbra_get_attachment_text(ctx, message_id: str, part: str, account_id: str = "") -> dict[str, Any]:
    ...
    @server.tool()
    async def zimbra_send_email(ctx, to: list[str], subject: str, body: str, account_id: str = "") -> dict[str, Any]:
