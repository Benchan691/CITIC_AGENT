...
import { removeApprovalMode, removeModelChoice } from "@/lib/config";
...
export function ThreadList({
  ...
  const handleDelete = useCallback(
    async (thread: ThreadItem) => {
      if (
        !window.confirm(
          `Delete “${thread.title}”? This will permanently remove the thread.`
        )
      ) {
        return;
      }
      ...
      try {
        ...
        removeApprovalMode(thread.id);
        removeModelChoice(thread.id);
        ...
        toast.error("Unable to delete thread", { description: message });
      } finally {
                  ...
                  <div className="flex flex-col gap-1">
                    {groupThreads.map((thread) => (
                      <div
                        ...
                        tabIndex={0}
                        className={cn(
                          "group flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-3 text-left transition-colors duration-200",
                          "hover:bg-accent",
                      ...
                      >
                        ...
                        <Button
                          ...
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 shrink-0 text-muted-foreground opacity-0 transition-opacity hover:text-destructive group-focus-within:opacity-100 group-hover:opacity-100"
