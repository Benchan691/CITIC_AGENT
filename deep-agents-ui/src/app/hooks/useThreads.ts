  ...
  return useSWRInfinite(
    ...
    async ({
      ...
      try {
        ...
        if (/fetch|connect|network|timeout|econnrefused/i.test(message)) {
          const unavailable = new Error(
            `Agent server unavailable at ${deploymentUrl}. Start the MCP server first, then restart LangGraph on this URL.`,
            { cause: reason }
          );
          throw unavailable;
        }
