 ...
 * @module dsh-soc-agent-session-persistence-jsonl
 */
...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  async deleteSession(id: SessionId): Promise<boolean> {
    ...
    preparation?.controller.abort(new Error(`session "${id}" is being deleted`))
    this.migrationPreparations.delete(id)
    this.coldLogMemo.delete(id)

    const selected = await this.findLog(id)
    if (selected === undefined) return hadLocalState
    const header = await this.readGenerationHeader(selected, id)
    if (header === undefined) {
      throw new SessionPersistenceCorruptionError(
        `session "${id}": stored generation has no valid header`,
        {},
      )
    }
    const directory = dirname(selected.sourcePath)
    if (basename(directory) !== encodeSegment(id)) {
      throw new SessionPersistenceCorruptionError(
        `session "${id}": refusing to delete unexpected directory ${JSON.stringify(directory)}`,
        {},
      )
    }
    ...
    if (withinRoot === ''
      ...
      || resolve(rootPath, withinRoot) !== directoryPath) {
      throw new SessionPersistenceCorruptionError(
        `session "${id}": refusing to delete a directory outside the configured data root`,
        {},
      )
    }
  ...
  private async listProjectDirs(signal?: AbortSignal): Promise<string[]> {
    try {
      ...
      signal?.throwIfAborted()
      return entries
        .filter(e => e.isDirectory() && e.name !== '.quarantine')
