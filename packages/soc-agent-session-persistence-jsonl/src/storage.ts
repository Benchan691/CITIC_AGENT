...
export class JsonlBackendTracker {
  ...
  async closeSession(id: SessionId): Promise<boolean> {
    ...
    if (failures.length > 0) {
      throw new AggregateError(failures, `session "${id}": active handles could not be closed for deletion`)
    }
