...
import { agentEvents, assembleContextFor, emitAgentEvent } from "dsh-soc-agent-agent";
...
* @module dsh-soc-agent-agent-loop/runtime-context
*/
...
var ReactLoopAgent = class {
	...
	sendBatch(messages, target, wakeup) {
	...
	*/
	async create(id, options = {}, meta = {}) {
		try {
			...
			const preparation = _usingCtx$1.u(SessionPreparation.create(this.runtime.ctx.sessions.prepare(id, { meta })));
	...
	*/
	...
	async setupAndPublish(ownerCtx, id, preparation, agentOptions, setup, signal, source, stored, parentAgent) {
		try {
			...
			const session = _usingCtx3.u(preparation).session;
			let prepared;
			try {
