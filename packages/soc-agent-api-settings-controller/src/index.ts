...
export class SettingsController extends TypertRemoteService {
  ...
  private readonly auth: SocAuth
  private readonly userNamespaces: ReadonlySet<string>
  ...
  constructor(ctx: Context, config: Config = {}, internals: SettingsControllerInternals = {}) {
    ...
    this.auth = ctx.get('socAuth' as never) as unknown as SocAuth
  ...
  describe(): SettingsDescribeValue {
    ...
    return {
      ...
      hasDocument: administrator && settings.documentPath !== undefined,
      namespaces: settings.describe({ redactSecrets: true })
        .filter(descriptor => administrator || this.userNamespaces.has(String(descriptor.ns)))
        .map(namespaceView),
    }
  }
  ...
  private provider(): SettingsProvider {
    ...
    if (settings === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'settings service is absent: this deployment does not mount a settings provider (e.g. dsh-soc-agent-settings-file) in its composition',
        {},
      )
    }
    return settings
  ...
  private requireNamespace(ns: string): void {
