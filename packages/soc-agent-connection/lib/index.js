...
//#endregion
//#region src/api-request-trust.ts
/**
...
var HostConnectionService = class extends Service {
	...
	*/
	constructor(ctx, trustedHosts, browserAuth, socAuth) {
	...
	authenticateUpgrade(request, audience) {
		return this.socAuth.authenticateUpgrade(request, audience);
	}
	runAs(principal, operation) {
		return this.socAuth.run(principal, operation);
	}
	revocationSignal(principal) {
		return this.socAuth.revocationSignal(principal);
	}
	...
	*/
	createSharedFetchHandler(channel) {
		return {
			...
			fetch: (request) => this.authenticatedFetch(request, async (authenticatedRequest) => {
				const pathname = new URL(authenticatedRequest.url).pathname;
				const route = this.fetchRoutes.get(pathname);
				if (route?.methods.has(authenticatedRequest.method) === true) return route.fetch(authenticatedRequest);
				...
				return interceptor.fetchHandler.fetch(authenticatedRequest);
			})
		};
	}
	async authenticatedFetch(request, dispatch) {
		let principal;
		try {
	...
	register(owner, channel, handler) {
		...
		const rawFetchHandler = rpcFetchHandler(channel, handler);
		const fetchHandler = {
			requestBodyMode: rawFetchHandler.requestBodyMode,
			fetch: (request) => this.authenticatedFetch(request, (authenticated) => rawFetchHandler.fetch(authenticated))
		};
		const route = {
			kind: "prefix",
			path: channel,
...
function authFailureStatus(error) {
	return typeof error === "object" && error !== null && Reflect.get(error, "status") === 403 ? 403 : 401;
}
...
const inject = ["credentials", "socAuth"];
...
async function apply(ctx, config) {
	...
	const connection = new HostConnectionService(ctx, trustedHosts, await BrowserAuth.create(ctx.root, ctx.credentials, cookieMaxAgeDays), ctx.get("socAuth"));
