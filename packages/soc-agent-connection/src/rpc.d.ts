...
/** Authenticated SOC request context captured at the transport boundary. */
export type SocPrincipal = {
    readonly kind: 'user';
...
/** Structural application-auth service supplied by the SOC Host. */
export interface SocAuth {
    authenticateHttp(request: Request, audience: 'user' | 'admin' | 'mixed'): Promise<SocPrincipal>;
    authenticateUpgrade(request: ConnectionIndexRequest, audience: 'user' | 'admin'): Promise<SocPrincipal>;
    run<T>(principal: SocPrincipal, operation: () => Promise<T> | T): Promise<T> | T;
    requireUser(): Extract<SocPrincipal, {
        readonly kind: 'user';
    }>;
    requireAdmin(): Extract<SocPrincipal, {
        readonly kind: 'admin';
    }>;
...
export interface HostConnectionHandle {
    ...
    /** Capture the authenticated user before admitting a WebSocket upgrade. */
    authenticateUpgrade(request: ConnectionIndexRequest, audience: 'user' | 'admin'): Promise<SocPrincipal>;
    /** Execute deferred transport work under its captured principal. */
    runAs<T>(principal: SocPrincipal, operation: () => Promise<T> | T): Promise<T> | T;
    /** Cancellation signal fired when the captured application session is revoked. */
    revocationSignal(principal: SocPrincipal): AbortSignal;
    /**
