...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  constructor(
    ...
    private readonly socAuth: SocAuth,
  ) {
  ...
  authenticateUpgrade(
    ...
    audience: 'user' | 'admin',
  ): Promise<SocPrincipal> {
    return this.socAuth.authenticateUpgrade(request, audience)
  }

  runAs<T>(principal: SocPrincipal, operation: () => Promise<T> | T): Promise<T> | T {
    return this.socAuth.run(principal, operation)
  }

  revocationSignal(principal: SocPrincipal): AbortSignal {
