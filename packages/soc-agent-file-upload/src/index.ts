...
import type {} from 'dsh-soc-agent-connection'
...
interface SocUploadPrincipal {
  readonly kind: 'user'
  readonly applicationSessionId: string
  readonly userId: string
}

interface SocUploadAuth {
  requireUser(): SocUploadPrincipal
...
export class FileUploads extends TypertRemoteService {
  static inject = ['agents', 'attachments', 'commands', 'connection', 'socAuth']

  private readonly stagedFiles = new WeakMap<Session, Map<FileUploadReceiptId, StagedFileUpload>>()
  private readonly auth: SocUploadAuth
  ...
  constructor(ctx: Context) {
    ...
    this.auth = ctx.get('socAuth' as never) as unknown as SocUploadAuth
    const resolve: CommandFileReceiptResolver = (agent, receiptId) =>
      this.resolve(agent as Agent, receiptId as FileUploadReceiptId)
    ctx.effect(
  ...
  async uploadStream(request: {
    ...
    if (!await this.auth.ownsSession(request.sessionId)) throw sessionNotFound(request.sessionId)
  ...
  resolve(agent: Agent, receiptId: FileUploadReceiptId): FileAttachmentRef | undefined {
    ...
    const principal = this.auth.requireUser()
    const upload = this.stagedFiles.get(agent.session)?.get(receiptId)
    if (upload === undefined || upload.sessionId !== agent.id
      || upload.applicationSessionId !== principal.applicationSessionId
      || upload.userId !== principal.userId) return undefined
    return upload.file
  }
  ...
  bindPrompt(
    ...
    const bound = receiptIds.map((receiptId) => {
      ...
      if (upload === undefined || upload.sessionId !== agent.id
        || upload.applicationSessionId !== principal.applicationSessionId
        || upload.userId !== principal.userId) throw fileNotStaged()
  ...
  private async commit(agent: Agent, save: () => Promise<FileAttachmentRef>): Promise<FileUploadValue> {
    ...
    const principal = this.auth.requireUser()
    if (!await this.auth.ownsSession(agent.id)) throw sessionNotFound(agent.id)
    ...
    staged.set(receiptId, {
      file,
      applicationSessionId: principal.applicationSessionId,
  ...
  private requireFeature(): void {
    if (this.ctx.get('socAttachmentsFeature' as never) !== undefined) return
    throw new RemoteError(
      'session/attachment-invalid' as never,
      'Attachments are disabled for this deployment.',
      { reason: 'FEATURE_DISABLED' } as never,
    )
  }
...
function sessionNotFound(sessionId: SessionId): RemoteError {
