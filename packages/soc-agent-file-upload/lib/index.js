	...
	return class FileUploads extends _classSuper {
		...
		*/
		resolve(agent, receiptId) {
			...
			if (upload === void 0 || upload.sessionId !== agent.id || upload.applicationSessionId !== principal.applicationSessionId || upload.userId !== principal.userId) return void 0;
			return upload.file;
		}
		/**
		...
		*/
		bindPrompt(agent, receiptIds, requestId) {
			...
			const bound = receiptIds.map((receiptId) => {
				...
				if (upload === void 0 || upload.sessionId !== agent.id || upload.applicationSessionId !== principal.applicationSessionId || upload.userId !== principal.userId) throw fileNotStaged();
				return {
					upload,
		...
		*/
		...
		requireFeature() {
			if (this.ctx.get("socAttachmentsFeature") !== void 0) return;
			throw new RemoteError("session/attachment-invalid", "Attachments are disabled for this deployment.", { reason: "FEATURE_DISABLED" });
		}
