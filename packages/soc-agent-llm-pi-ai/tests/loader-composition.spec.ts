...
import FileSettingsProvider from 'dsh-soc-agent-settings-file'
import * as LlmPiAi from 'dsh-soc-agent-llm-pi-ai'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  ...
  await writeFile(configPath, [
    ...
    '- id: settings',
    "  name: 'dsh-soc-agent-settings-file'",
    '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['dsh-soc-agent-settings-file', FileSettingsProvider],
    ['@deepseek-ai/dsh-credentials-local', LocalCredentialProvider],
    ['dsh-soc-agent-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
