...
} from 'dsh-soc-agent-session-controller/client'
import type {} from 'dsh-soc-agent-file-upload/client'
import type { SessionId } from '@deepseek-ai/dsh-session/types'
import type { ImageMediaType } from '@deepseek-ai/dsh-attachment'
import { createSnapshotStore } from '@deepseek-ai/dsh-client-store'
import type { SnapshotStore } from '@deepseek-ai/dsh-client-store'
import type {
  ComposerAttachment, ComposerDocument, ComposerFileAttachment, ComposerImageAttachment, DraftFileUpload,
...
export interface IConversation {
  ...
  /** Install the single optional document-to-Markdown intake provider. */
...
/** Converted document text admitted as part of one user prompt. */
...
/** Optional document lifecycle owned by the SOC attachments plugin. */
...
export class ConversationController extends Service implements IConversation {
    ...
    ctx.effect(() => async () => {
      ...
      for (const attachment of this.draftAttachments.values()) {
        ...
        if (attachment.kind === 'document') {
          ...
          if (sessionId !== undefined) this.documentProvider?.release(sessionId, attachment.id)
        }
      }
  ...
  registerDocumentProvider(provider: ComposerDocumentProvider): () => void {
    if (this.documentProvider !== undefined) {
      throw new Error('conversation: a document provider is already registered')
    }
    ...
    return () => {
      ...
      active = false
      for (const attachment of [...this.draftAttachments.values()]) {
        if (attachment.kind === 'document') this.releaseDraftAttachment(attachment.id)
      }
      this.documentProvider = undefined
    }
  }
  ...
  async sendSession(
    ...
    const pendingAttachments = attachments.flatMap(attachment => attachment.kind === 'document'
      ? []
    ...
    const serializeAttachments = async (): Promise<Parameters<SessionFace['prompt']>[0]> => {
      const documents = attachments.filter((attachment): attachment is ComposerDocument => attachment.kind === 'document')
      const converted = documents.length === 0
        ? []
        : await this.requireDocumentProvider().convert(
          session.sessionId,
          documents.map(document => document.id),
          signal ?? new AbortController().signal,
        )
      const convertedById = new Map(converted.map(document => [document.id, document] as const))
      return Promise.all(attachments.map(async (attachment) => {
        if (attachment.kind === 'image') return { type: 'image' as const, ...await this.encodeImage(attachment.file) }
        if (attachment.kind === 'file') return { type: 'file' as const, receiptId: uploadFor(attachment).receiptId }
        const document = convertedById.get(attachment.id)
        if (document === undefined) throw new Error(`conversation: document conversion produced no result for ${attachment.file.name}`)
        return {
          type: 'text' as const,
  ...
  async serializeDraftAttachments(
    ...
    return {
      attachments: await Promise.all(attachments.map(async (attachment) => {
        if (attachment.kind === 'image') return { type: 'image' as const, ...await this.encodeImage(attachment.file) }
        if (attachment.kind === 'document') {
          throw new Error('conversation.serializeDraftAttachments: document drafts require normal composer submission')
        }
        const upload = uploads[attachment.id]
        if (upload === undefined || upload.status !== 'ready') {
  ...
  releaseDraftAttachment(id: DraftAttachmentId): void {
    ...
    if (attachment.kind === 'document') {
      if (sessionId !== undefined) this.documentProvider?.release(sessionId, id)
      return
    }
  ...
  private requireDocumentProvider(): ComposerDocumentProvider {
    if (this.documentProvider === undefined) {
      throw new Error('conversation: document attachment support is disabled')
    }
