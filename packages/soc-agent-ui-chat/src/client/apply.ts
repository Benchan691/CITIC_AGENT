...
import type { SessionBinding } from 'dsh-soc-agent-session-controller/client'
...
import type {} from 'dsh-soc-agent-ui-session/client'
import type {} from '@deepseek-ai/dsh-client-ui-settings/client'
import type {} from 'dsh-soc-agent-workspace/client'
import type {
...
export function apply(ctx: Context): void {
  ...
  const autoCollapse = (ctx.get('socAutoCollapse' as never) as
    | { state: SnapshotStore<{ enabled: boolean; statusText: string }> }
    | undefined)?.state
    ?? createSnapshotStore({ enabled: false, statusText: '' })
  ...
  ctx.slots.inject('conversation.view', () => {
    const disposeView = ctx.slots.register({
      ...
      inject: (sessionId: SessionId): ChatViewInjected => {
        ...
        return {
          hooks: { transcriptView: transcriptView.mode, autoCollapse },
          keyedHooks: {
