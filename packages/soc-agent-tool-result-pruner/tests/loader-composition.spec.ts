...
import ToolResultPruner from 'dsh-soc-agent-tool-result-pruner'
...
describe('compaction-tool-result-pruner real Loader composition', () => {
  it('loads and resolves the flat YAML plugin shape', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: 'dsh-soc-agent-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        ...
        if (specifier === 'dsh-soc-agent-tool-result-pruner') return ToolResultPruner
