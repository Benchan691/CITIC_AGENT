...
import * as workspaceContext from 'dsh-soc-agent-agent-instructions'
...
import AgentRegistry, { agentEvents, type Agent } from 'dsh-soc-agent-agent'
import AgentLoop, { turnBoundaryProjectionDefinition } from 'dsh-soc-agent-agent-loop'
...
} from 'dsh-soc-agent-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('dsh-soc-agent-agent-instructions')
