window.__ModuleLoader__.load({
	id: "dsh-soc-agent-workspace-controller",
	factory: (require) => {
		var module = { exports: {} };
		var exports = module.exports;
		Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
		let dsh_soc_agent_api_gateway_client = require("dsh-soc-agent-api-gateway/client");
		...
		var ClientWorkspaceModel = class {
			...
			*/
			handleStreamFailure(error) {
				if (!(0, dsh_soc_agent_api_gateway_client.isRemoteFailure)(error)) throw error;
				this.state = "error";
				this.error = error;
				this.invalidate();
			}
		...
		*/
		function createWorkspaceStateStream(remote, options) {
			return new dsh_soc_agent_api_gateway_client.RemoteSnapshotStream(remote.$stream({
				name: "Workspace state stream",
				open: (signal) => remote.workspace.follow(signal),
				ended: (accepted) => accepted ? new dsh_soc_agent_api_gateway_client.RemoteStreamCarrierError("Workspace state stream ended without a terminal result") : /* @__PURE__ */ new Error("Workspace state stream ended before its opening snapshot"),
