...
var WorkspaceCommands = class {
	...
	*/
	delete(request) {
		return this.enqueue(async () => {
			if (!await this.ctx.workspaceRegistry.delete(WorkspaceId(request.workspaceId))) throw workspaceNotFound$1(request.workspaceId);
			return { deleted: true };
		});
	}
	/**
	...
	*/
	async insertBefore(request) {
		try {
			...
			throw workspaceNotFound$1(error.workspaceId);
		}
	}
	/**
	...
	*/
	...
	requireWorkspace(workspaceId) {
		...
		if (workspace === void 0) throw workspaceNotFound$1(workspaceId);
		return workspace;
	}
	enqueue(operation) {
...
function workspaceNotFound$1(workspaceId) {
	...
	return class WorkspaceController extends _classSuper {
		...
		*/
		async create(request) {
			...
			if (!await auth.claimWorkspace(value.workspace.workspaceId, path)) throw workspaceNotFound(String(value.workspace.workspaceId));
			return value;
		}
		/**
		...
		*/
		async rename(request) {
			...
			if (await this.auth?.isGeneralWorkspace(request.workspaceId)) throw new RemoteError("workspace/name-conflict", "General is protected", { name: "General" });
		...
		*/
		async insertBefore(request) {
			await this.requireWorkspace(request.workspaceId);
			if (request.beforeWorkspaceId !== void 0) await this.requireWorkspace(request.beforeWorkspaceId);
			const value = await this.commands.insertBefore(request);
			const allowed = await this.auth?.ownedWorkspaceIds();
			return allowed === void 0 ? value : { workspaceIds: value.workspaceIds.filter((id) => allowed.has(String(id))) };
		}
		/**
		...
		*/
		async insertSessionBefore(request) {
			...
			if (request.beforeSessionId !== void 0) await this.requireSession(request.beforeSessionId);
		...
		*/
		async archiveSession(request) {
			...
			return allowed === void 0 ? value : { archivedSessionIds: value.archivedSessionIds.filter((id) => allowed.has(String(id))) };
		}
		/**
...
function sessionNotFound(sessionId) {
	return new RemoteError("session/not-found", "session not found", { sessionId });
}
async function filterWorkspaceFrame(frame, auth, visible) {
	...
	if (frame.type === "baseline") {
		...
		return {
			type: "baseline",
			value: {
				items: frame.value.items.filter((item) => visible.has(String(item.workspaceId))).map((item) => ({
					...item,
					sessionIds: item.sessionIds.filter((id) => sessionIds.has(String(id)))
				})),
	...
	if (frame.type === "upsert") {
		if (!await auth.ownsWorkspace(frame.workspace.workspaceId)) return void 0;
	...
	if (frame.type === "remove") {
		if (!visible.delete(String(frame.workspaceId))) return void 0;
		return frame;
	}
	if (frame.type === "order") return {
		...frame,
		workspaceIds: frame.workspaceIds.filter((id) => visible.has(String(id)))
	};
	return {
		...frame,
		archivedSessionIds: frame.archivedSessionIds.filter((id) => sessionIds.has(String(id)))
	};
}
//#endregion
