...
侧边栏外壳插件：负责品牌行、New Session 操作、布局持有的折叠控件、可感知滚动的区域 seat，以及固定在底部的 Settings seat。[soc-agent-workspace](../soc-agent-workspace/README.zh.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；本包既不派生其中的行，也不持有其视图偏好。折叠到布局拥有的 56px 轨道仍属于本地呈现行为。约定：[slot 系统标准](../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.zh.md)。

这是当前 Harness 侧边栏的 SOC 固定快照。本包独立持有标准 sidebar slot 及其子 slot 约定；SOC 应用会停用上游 sidebar 行并启用本包。运行时、布局、locale、slot、primitive 与设置等共享合约仍是兼容边界。Harness 的样式更新必须显式修改本包，不会自动同步源码。
...
New Session 会启动运行时的页面局部前端 Session Intent。运行时优先使用作用域操作明确指定的 Workspace，否则使用当前 Session 所属 Workspace，再否则使用最近活跃 Workspace；一个 Workspace 都没有时则清空选择，进入空白 New Session 页面。Workspace 专属控件与共享选择器由 soc-agent-workspace 持有。
...
栏内的滚动条是一种指针可供性：只要指针不在栏内，外壳就把 ui-theme 的[滚动条间接层](../../vendor/deepseek-harness/packages/client/ui-theme/README.zh.md)重新绑定为 `transparent`；指针离开后滑块再保留 2 秒，因此没人指向的列表不会带着滚动条。避免行位移的空间预留属于滚动区域本身（[soc-agent-workspace](../soc-agent-workspace/README.zh.md)），所以显示滑块不会引起重排。
...
- **Session 状态点渲染由 [soc-agent-workspace](../soc-agent-workspace/README.zh.md) 持有**：没有可用的 done/error 通知数据源。
- **Workspace 浏览行为由组合持有**：分组、排序、搜索与行状态都属于 [soc-agent-workspace](../soc-agent-workspace/README.zh.md)，不属于此外壳。
