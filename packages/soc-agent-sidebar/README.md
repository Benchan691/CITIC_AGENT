...
Sidebar shell plugin: the brand row, New Session action, layout-owned collapse control, scroll-aware region seat, and bottom-pinned Settings seat. [soc-agent-workspace](../soc-agent-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).

This is a pinned SOC snapshot of the current Harness sidebar. It owns the standard sidebar slot and its child-slot contract independently; the SOC application disables the upstream sidebar row and enables this package. Shared runtime, layout, locale, slot, primitive, and settings contracts remain compatibility boundaries. Updating Harness styling is an explicit package change, not an automatic source sync.
...
Scrollbars in the column are a pointer affordance: the shell rebinds ui-theme's [scrollbar indirection](../../vendor/deepseek-harness/packages/client/ui-theme/README.md) to `transparent` whenever the pointer is outside it, and keeps the thumb drawn for 2s after the pointer leaves, so a list nobody is pointing at carries no bar. The reservation that keeps rows from moving belongs to the scrolling region ([soc-agent-workspace](../soc-agent-workspace/README.md)), so revealing a thumb never reflows.
...
- **Session state-dot rendering is owned by [soc-agent-workspace](../soc-agent-workspace/README.md)** — no done/error notification sources are available.
- **Workspace browser behavior is composition-owned** — grouping, ordering, search, and row state belong to [soc-agent-workspace](../soc-agent-workspace/README.md), not this shell.
