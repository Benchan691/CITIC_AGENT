...
export const inject = ['commands', 'connection', 'socAuth']
...
async function sessionLogExportResponse(
  ...
  const auth = ctx.get('socAuth' as never) as unknown as SocSessionLogAuth
  auth.requireUser()
  if (!await auth.ownsSession(sessionId)) return new Response('session not found', { status: 404 })
  ...
  if (descendantsValue === 'true') {
    const lineage = await ready.sessionQuery.traceSession(sessionId, request.signal)
    const pending = [...lineage.descendants]
    while (pending.length > 0) {
      const node = pending.pop()
      if (node === undefined) continue
      if (!await auth.ownsSession(node.session.header.id)) return new Response('session not found', { status: 404 })
