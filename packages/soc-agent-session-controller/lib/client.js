window.__ModuleLoader__.load({
	id: "dsh-soc-agent-session-controller",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		...
		//#endregion
		//#region src/client/session-wire-event.ts
		...
		*/
		function createSessionControlStream(remote, options) {
			return new dsh_soc_agent_api_gateway_client.RemoteSnapshotStream(remote.$stream({
				...
				ended: (accepted) => accepted ? new dsh_soc_agent_api_gateway_client.RemoteStreamCarrierError("session control stream ended without a terminal result") : /* @__PURE__ */ new Error("session control stream ended before its opening snapshot"),
		...
		var SessionEventStream = class extends dsh_soc_agent_api_gateway_client.RemoteJournalStream {
			remote;
			address;
			/**
			* @param remote - generated Session namespace and Gateway stream factory.
			...
			async *follow(request, signal) {
				...
				for await (const frame of this.remote.session.follow({
					...
					if (frame.type === "assistant-stream") {
						...
						if (frame.frame.revision !== expected) throw new dsh_soc_agent_api_gateway_client.RemoteStreamCarrierError(`session assistant stream skipped revision ${String(expected)}`);
		...
		*/
		...
		//#endregion
		//#region src/client/sessions/assistant-stream.ts
		...
		//#endregion
		//#region src/client/sessions/session.ts
			...
			*/
			...
			async loadOlder() {
				...
				try {
					...
					if (!(0, dsh_soc_agent_api_gateway_client.isRemoteFailure)(error)) console.error("[session-controller] loadOlder failed:", error);
				} finally {
			...
			loadThrough(seq) {
				...
				this.jumpPromise = (async () => {
					try {
						...
						if (!(0, dsh_soc_agent_api_gateway_client.isRemoteFailure)(error)) console.error("[session-controller] loadThrough failed:", error);
					} finally {
		...
		var ClientSessions = class {
			...
			*/
			constructor(rootCtx, remote) {
				...
				this.manager = new SessionManager(this.remote, restored.sessionId, restored.subagentAddress);
		...
		*/
		...
		exports.SessionDeleteError = SessionDeleteError;
