...
import { installModelSelection } from "dsh-soc-agent-agent";
...
async function inspectApiSession(ctx, sessionId, signal) {
	try {
		try {
			...
			const observation = _usingCtx$3.u(await ctx.sessionQuery.observeSession(sessionId, {
...
var ApiSessionAgentController = class {
	...
	/** Close the live Agent owned by this controller before durable deletion. */
	...
	/** Retain the lifecycle capability for every ordinary Session activated here. */
	retain(handle) {
	...
	async resume(sessionId, supplied) {
		...
		try {
			try {
				...
				const observation = _usingCtx3.u(await this.ctx.sessionQuery.observeSession(sessionId));
	...
	async createOrAdopt(sessionId, cwd, checkPersistedIdentity, presetId) {
		...
		if (checkPersistedIdentity) try {
			try {
				...
				const observation = _usingCtx4.u(await this.ctx.sessionQuery.observeSession(sessionId));
				if (hasApiSessionSubagentOwner(this.ctx, { header: observation.header }, void 0)) throw new ApiSessionSubagentOwnership(sessionId);
	...
	*/
	async fork(request) {
		try {
			...
			const source = _usingCtx$2.u(observed);
		...
		const admit = async () => {
			try {
				try {
					...
					const binding = _usingCtx3.u(this.ctx.fileUploads.bindPrompt(agent, admission.receiptIds, request.requestId));
...
//#endregion
//#region src/assistant-stream.ts
...
const MESSAGE_TYPES$1 = /* @__PURE__ */ new Set(["user/message", "assistant/message"]);
	...
	*/
	async page(request, signal) {
		try {
			...
			const source = _usingCtx$1.u(await this.sourceFor(request.address, signal, false));
			signal.throwIfAborted();
	...
	*/
	async *follow(request, signal) {
		...
		try {
			try {
				var _usingCtx3 = _usingCtx();
				const source = _usingCtx3.u(await this.sourceFor(address, signal, true));
...
var ApiSessionList = class {
	...
	*/
	async list(allowedIds, signal) {
		signal?.throwIfAborted();
		const records = allowedIds === void 0 ? await this.ctx.sessionQuery.listSessions(signal) : allowedIds.size === 0 ? [] : await this.ctx.sessionQuery.filterSessions([{
			kind: "id",
			values: [...allowedIds]
		}], signal);
		signal?.throwIfAborted();
	...
	*/
	async search(query, signal, allowedIds) {
		...
		try {
			const visibleIds = allowedIds === void 0 ? new Set((await provider.listSessions(signal)).filter((record) => record.header.cwd !== void 0).map((record) => record.header.id)) : /* @__PURE__ */ new Set([...allowedIds]);
			signal.throwIfAborted();
...
//#endregion
//#region src/model-selection-projection.ts
...
//#endregion
//#region src/media-references.ts
/**
...
* @module dsh-soc-agent-session-controller/media-references
*/
const BASE_HEADERS = {
...
let SessionController = (() => {
	...
	return class SessionController extends _classSuper {
		...
		*/
		async create(request) {
			const auth = this.auth;
			if (auth === void 0) return this.commands.create(request);
			const principal = auth.requireUser();
			if (request.cwd !== void 0) throw new RemoteError("gateway/bad-request", "SOC sessions must belong to an owned workspace", {});
			const workspaceId = request.workspaceId ?? brandString((await auth.ensureGeneral(principal.userId)).workspaceId);
			...
			try {
				...
				const value = await this.commands.create({
					...safeRequest,
					workspaceId,
					sessionId
				});
				...
				if (!(error instanceof RemoteError) || error.code !== "session/workspace-attach-failed") await auth.releaseSession(sessionId);
				throw error;
			}
		}
		/**
		...
		*/
		async delete(request) {
			...
			if (!closedLiveOwner && this.ctx.agents.get(sessionId) !== void 0) throw new RemoteError("session/agent-busy", `session "${sessionId}" is owned by another live Agent controller`, { reason: "live-owner-not-held-by-session-controller" });
			const persistence = this.ctx.sessionPersistence;
			if (typeof persistence.deleteSession !== "function") throw new RemoteError("gateway/internal", "configured Session persistence does not support permanent deletion", {});
			if (!await persistence.deleteSession(sessionId)) throw new RemoteError("session/not-found", `session "${sessionId}" not found`, { sessionId });
			await Promise.all(this.ctx.workspaceRegistry.list().filter((workspace) => workspace.sessionIds.includes(sessionId)).map((workspace) => workspace.detachSession(sessionId)));
		...
		*/
		async fork(request) {
			...
			if (auth !== void 0) {
				...
				if (!workspaceId || !await auth.claimSession(value.sessionId, workspaceId)) throw sessionNotFound(request.sessionId);
		...
		*/
		async *control(signal) {
			...
			for await (const frame of this.controlState.control(signal)) {
				const filtered = filterControlFrame(frame, await auth.ownedSessionIds());
...
function filterControlFrame(frame, allowed) {
	if (frame.type !== "baseline") return "sessionId" in frame && allowed.has(String(frame.sessionId)) ? frame : void 0;
	const filterRecord = (record) => Object.fromEntries(Object.entries(record).filter(([sessionId]) => allowed.has(sessionId)));
	return {
		...frame,
		value: {
			...frame.value,
