...
import { SESSION_SEARCH_RESULT_LIMIT, type SessionDeleteValue } from '../../types.ts'
...
/** Structured permanent-deletion failure retaining the Remote error code. */
...
type SessionDeletionRemote = SessionRemotes['session'] & {
  delete(
...
export class ClientSessions implements ISessions {
  ...
  /** Permanently delete a Session and converge the local list immediately. */
  async delete(id: SessionId): Promise<void> {
    const result = await (this.remote.session as SessionDeletionRemote).delete({ sessionId: id })
    if (!result.ok) throw new SessionDeleteError(result.error, id)
    this.handleSessionRemoved(id)
  }
