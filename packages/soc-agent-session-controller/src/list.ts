...
export class ApiSessionList {
   ...
   */
  async list(allowedIds?: ReadonlySet<string>, signal?: AbortSignal): Promise<SessionSummary[]> {
    signal?.throwIfAborted()
    const records = allowedIds === undefined
      ? await this.ctx.sessionQuery.listSessions(signal)
      : allowedIds.size === 0
        ? []
        : await this.ctx.sessionQuery.filterSessions([
          { kind: 'id', values: [...allowedIds] as SessionId[] },
        ], signal)
    signal?.throwIfAborted()
  ...
  async search(
    ...
    try {
      const visibleIds = allowedIds === undefined
        ? new Set((await provider.listSessions(signal))
          .filter(record => record.header.cwd !== undefined)
          .map(record => record.header.id))
        : new Set([...allowedIds] as SessionId[])
      signal.throwIfAborted()
