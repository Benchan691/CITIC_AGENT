...
function rpcErrorCode(reason: unknown): string | undefined {
  ...
  const value = reason as { code?: unknown; rpcError?: { code?: unknown } }
                ...
                actions={group.workspaceId === undefined
                  ...
                  : workspaces.find(item => item.workspaceId === group.workspaceId)?.title === 'General'
                    ? {
                      ...
                      clearDisabled: (workspaces.find(item => item.workspaceId === group.workspaceId)
...
function FlatList({
  useSessions, useSessionPendingInteraction, open, forkSession, onSessionRename, onSessionDelete, onSessionArchive,
...
export function WorkspaceBrowser({
  ...
  type ClearTarget = {
    workspaceId: WorkspaceId
    title: string
    /** Membership captured from the authenticated user's displayed Workspace. */
    sessionIds: readonly SessionId[]
    totalSessionCount: number
    remainingSessionIds: readonly SessionId[]
    deletedCount: number
  }
  ...
  const confirmClear = async () => {
    ...
    for (const sessionId of target.remainingSessionIds) {
      ...
      try {
        ...
        // This exception is safe only because `sessionId` came from the
        // displayed membership snapshot captured when this dialog opened.
  ...
  const onClearRequest = (
    workspaceId: WorkspaceId,
    title: string,
    sessionIdsToDelete: readonly SessionId[],
  ) => {
  ...
  return (
    <div className={clsx(css.root, !wide && css.rail)}>
      ...
      >
        {sessionDeleting && (
          <div className={css.deleteStatus} role="status">{t('delete.session.pending')}</div>
        )}
