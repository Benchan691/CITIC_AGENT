 ...
 * `workspaceFiles`.
 *
 * Every operation is authorized by the selected Session and confined to that
 * authenticated user's private workspace root. Absolute escapes, traversal,
 * symlinks, and out-of-root reads are rejected.
 *
...
interface SocWorkspaceFileAuth {
  requireUser(): unknown
  ownsSession(sessionId: SessionId | string): Promise<boolean>
  workspacePathForSession(sessionId: SessionId | string): Promise<string | undefined>
}
...
export class WorkspaceFiles extends TypertRemoteService {
  static inject = ['fs', 'socAuth', 'typert']
  ...
  private readonly auth: SocWorkspaceFileAuth
  ...
  constructor(ctx: Context, private readonly config: Config) {
    ...
    this.auth = ctx.get('socAuth' as never) as unknown as SocWorkspaceFileAuth
    ...
    ctx.inject(['typert'], (scope) => {
      scope.typert.lookups.register('workspaceFileScope', {
        ...
        hostTypeSymbol: 'dsh-soc-agent-api-workspace-files#WorkspaceFileScope',
        wireTypeSymbol: '@deepseek-ai/dsh-session/types#SessionId',
        resolve: async (sessionId) => {
          this.auth.requireUser()
          if (!await this.auth.ownsSession(sessionId)) return undefined
          const workspaceRoot = await this.auth.workspacePathForSession(sessionId)
