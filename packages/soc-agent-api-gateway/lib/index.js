...
function projectRemoteEventRequest(value, subject) {
	...
	for (const key of Reflect.ownKeys(value)) {
		...
		const descriptor = typeof key === "string" ? Object.getOwnPropertyDescriptor(value, key) : void 0;
		if (typeof key !== "string" || descriptor?.enumerable !== true) throw new TypeError("api gateway: Remote event request has a non-JSON property");
...
* @module dsh-soc-agent-api-gateway
*/
			...
			webCtx.effect(() => {
				const route = {
					...
					handler: async (req, socket, head) => {
						...
						try {
							const principal = await webCtx.connection.authenticateUpgrade(req, "user");
							const revocation = webCtx.connection.revocationSignal(principal);
							const revoke = () => {
								socket.destroy();
							};
							revocation.addEventListener("abort", revoke, { once: true });
							socket.once("close", () => {
								revocation.removeEventListener("abort", revoke);
							});
							await webCtx.connection.runAs(principal, async () => {
								mux.handleUpgrade(req, socket, head);
							});
						} catch (error) {
							rejectRemoteStreamUpgrade(socket, typeof error === "object" && error !== null && Reflect.get(error, "status") === 403 ? 403 : 401);
						}
					}
				};
	...
	*/
	...
	async dispatchRpc(endpoint, payload, signal) {
		if (endpoint === "$events/result") try {
			...
			if (this.requireUserPrincipal().applicationSessionId !== client.principal.applicationSessionId) throw new Error("typert gateway: Remote event result identifies no active event stream");
	...
	async *openRemoteEvents(payload, signal) {
		...
		for (const pending of this.pendingRemoteEvents.values()) if (this.mayDeliverRemoteEvent(pending, client)) this.deliverRemoteEvent(pending, client);
		try {
			yield {
	...
	broadcastRemoteEvent(frame) {
		...
		for (const client of this.remoteEventClients.values()) {
			if (frame.principal !== void 0 && frame.principal.applicationSessionId !== client.principal.applicationSessionId) continue;
	...
	startRemoteEvent(source) {
		try {
			...
			else for (const client of this.remoteEventClients.values()) if (this.mayDeliverRemoteEvent(pending, client)) this.deliverRemoteEvent(pending, client);
		} catch (error) {
			source.reject(error);
		}
	}
	...
	mayDeliverRemoteEvent(pending, client) {
		return pending.source.context.principal.applicationSessionId === client.principal.applicationSessionId;
	}
	requireUserPrincipal() {
		const auth = this.ctx.get("socAuth");
		if (auth === void 0) throw new Error("typert gateway: authenticated request context is unavailable");
