...
def _csv(value: str, default: str = "") -> tuple[str, ...]:
    raw = _value({"value": value}, "value", default)
...
class ZimbraSettings:
    ...
    # Kept only for backwards-compatible unit construction; environment loading no longer uses them.
    email: str = ""
    password: str = ""
...
class ServerSettings:
    ...
    def from_env(cls, values: Mapping[str, str] | None = None) -> "ServerSettings":
        ...
        zimbra = ZimbraSettings(
            ...
            accounts_file=_value(env, "ZIMBRA_ACCOUNTS_FILE", ".data/zimbra_accounts.enc"),
            key_file=_value(env, "ZIMBRA_ACCOUNTS_KEY_FILE", ".data/zimbra_accounts.key"),
            explicit_key=_value(env, "ZIMBRA_ACCOUNTS_KEY"),
            account_api_key=_value(env, "ZIMBRA_ACCOUNT_API_KEY"),
        ...
        return cls(
            ...
            zimbra=zimbra,
            app_postgres_uri=(
                _value(env, "APP_POSTGRES_URI")
                or _value(env, "LANGGRAPH_POSTGRES_URI")
                or _value(env, "POSTGRES_URI")
            ),
            settings_encryption_key=_value(env, "APP_SETTINGS_ENCRYPTION_KEY"),
        )
